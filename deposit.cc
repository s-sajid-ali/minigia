        ...
        bool zero_first)
{
    ...
    double weight0 = (bunch.get_real_num() / bunch.get_total_num())
            * bunch.get_reference_particle().get_charge() * pconstants::e
            / (h[0] * h[1] * h[2]);
    int ix, iy, iz;
...
deposit_charge_rectangular_zyx_eigen(
        Rectangular_grid_eigen<double> & rho, 
        Rectangular_grid_domain_eigen const & domain,
        Bunch const & bunch,
        bool zero_first )
{

    if (zero_first) rho.set_zero();

    auto const & parts = bunch.get_local_particles();
    auto const & h = domain.get_cell_size();
    auto const & shape = rho.shape();
    ...
    else 
    {
        for (int n = 0; n < bunch.get_local_num(); ++n) 
        {
            // domain doesn't know about xyz->zyx transformation, so we
            // do it in the order of arguments here
            domain.get_leftmost_indices_offsets(
                    parts(n, 4), parts(n, 2), parts(n, 0), 
                    iz, iy, ix, offz, offy, offx);
                ...
                for (int j = 0; j < 2; ++j) 
                {
                    for (int k = 0; k < 2; ++k) 
                    {
                                ...
                                && (cellz < shape[0])) 
                        {
                            ...
                            rho.grid(cellz, celly, cellx) += weight;
                        }
                    }
                }
            }
