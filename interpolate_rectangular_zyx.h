...
interpolate_rectangular_zyx(double x, double y, double z,
        Rectangular_grid_domain const & domain, 
        MArray3d_ref const& a)
{
        ...
        Rectangular_grid_eigen<double> const & a)
{
    ...
    double val;
    if (in_domain) {
        val = (   (1.0 - offz) * (1.0 - offy) * (1.0 - offx) * a.grid(iz, iy, ix)
                + offz * (1.0 - offy) * (1.0 - offx) * a.grid(iz + 1, iy, ix)
                + (1.0 - offz) * offy * (1.0 - offx) * a.grid(iz, iy + 1, ix)
                + (1.0 - offz) * (1.0 - offy) * offx * a.grid(iz, iy, ix + 1)
                + offz * offy * (1.0 - offx) * a.grid(iz + 1, iy + 1, ix)
                + offz * (1.0 - offy) * offx * a.grid(iz + 1, iy, ix + 1)
                + (1.0 - offz) * offy * offx * a.grid(iz, iy + 1, ix + 1)
                + offz * offy * offx * a.grid(iz + 1, iy + 1, ix + 1) );
    } else {
        val = 0.0;
    }
    return val;
