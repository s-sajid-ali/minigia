...
unsigned long int Random_distribution::get_default_seed(const char *device) {
...
void Random_distribution::set_seed(unsigned long int seed) {
  if (seed == 0) {
...
unsigned long int Random_distribution::get_original_seed() const {
  return original_seed;
}

double Random_distribution::get() { return gsl_rng_uniform(rng); }

double Random_distribution::get_uniform(double min, double max) {
  return gsl_ran_flat(rng, min, max);
}

double Random_distribution::get_unit_gaussian() {
...
Random_distribution::~Random_distribution() { gsl_rng_free(rng); }
