...
void Four_momentum::update_from_gamma() {
  if (gamma < 1.0) {
...
Lsexpr Four_momentum::as_lsexpr() const {
  Lsexpr retval;
...
void Four_momentum::set_total_energy(double total_energy) {
  gamma = total_energy / mass;
  update_from_gamma();
}

void Four_momentum::set_kinetic_energy(double kinetic_energy) {
  gamma = (mass + kinetic_energy) / mass;
  update_from_gamma();
}

void Four_momentum::set_momentum(double momentum) {
  double r2 = momentum * momentum / (mass * mass);
  beta = sqrt(r2 / (1 + r2));
  set_beta(beta);
}

void Four_momentum::set_gamma(double gamma) {
  this->gamma = gamma;
  update_from_gamma();
}

void Four_momentum::set_beta(double beta) {
...
double Four_momentum::get_mass() const { return mass; }

double Four_momentum::get_total_energy() const { return energy; }

double Four_momentum::get_kinetic_energy() const { return energy - mass; }

double Four_momentum::get_momentum() const { return momentum; }

double Four_momentum::get_gamma() const { return gamma; }

double Four_momentum::get_beta() const { return beta; }

bool Four_momentum::equal(Four_momentum const &four_momentum,
