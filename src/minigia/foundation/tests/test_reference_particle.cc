...
TEST_CASE("construct2_lsexpr") {
  ...
  for (int i = 0; i < 6; ++i)
    CHECK(from_lsexpr.get_state()[i] == Approx(0.0).margin(tolerance));
...
TEST_CASE("construct3_lsexpr") {
  ...
  CHECK(from_lsexpr.get_s_n() == Approx(partial_s).margin(tolerance));
}
...
TEST_CASE("get_state") {
  ...
  for (int i = 0; i < 6; ++i)
    CHECK(reference_particle.get_state()[i] == Approx(0.0).margin(tolerance));
}
...
TEST_CASE("set_trajectory") {
  ...
  CHECK(reference_particle.get_repetition_length() ==
        Approx(steps * step_length).margin(tolerance));
  CHECK(reference_particle.get_s_n() == Approx(partial_s).margin(tolerance));
}
...
void kick_reference_particle_state(Reference_particle &rp) {
