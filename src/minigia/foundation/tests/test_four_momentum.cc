...
TEST_CASE("construct") { REQUIRE_NOTHROW(Four_momentum(mass)); }

TEST_CASE("construct2") { REQUIRE_NOTHROW(Four_momentum(mass, total_energy)); }
...
TEST_CASE("lsexpr") {
  ...
  CHECK(from_lsexpr.get_total_energy() ==
...
TEST_CASE("get_total_energy") {
  Four_momentum four_momentum(mass);
  CHECK(four_momentum.get_total_energy() == Approx(mass).margin(tolerance));
}
...
TEST_CASE("get_gamma") {
  Four_momentum four_momentum(mass);
  four_momentum.set_total_energy(total_energy);
  CHECK(four_momentum.get_gamma() == Approx(my_gamma).margin(tolerance));
}

TEST_CASE("get_beta") {
  Four_momentum four_momentum(mass);
  four_momentum.set_total_energy(total_energy);
  CHECK(four_momentum.get_beta() == Approx(beta).margin(tolerance));
}
