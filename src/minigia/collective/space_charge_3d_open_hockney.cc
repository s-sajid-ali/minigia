...
namespace {
void print_grid(Logger &logger, karray1d_dev const &grid, int x0, int x1,
                int y0, int y1, int z0, int z1, int gx, int gy, int gz,
                int off = 0) {
...
struct alg_zeroer {
  ...
  KOKKOS_INLINE_FUNCTION
  void operator()(const int i) const { arr(i) = 0.0; }
};
...
struct alg_g2_pointlike {
  ...
  alg_g2_pointlike(karray1d_dev const &g2, std::array<int, 3> const &g,
                   std::array<int, 3> const &dg, std::array<double, 3> const &h)
      : g2(g2), gx(g[0] + 1), gy(g[1] + 1), dgx(dg[0]), dgy(dg[1]), dgz(dg[2]),
        padded_dgx(Distributed_fft3d::get_padded_shape_real(dgx)), hx(h[0]),
        hy(h[1]), hz(h[2]), igxgy(1.0 / (gx * gy)), igx(1.0 / gx),
...
struct alg_g2_linear {
  ...
  alg_g2_linear(karray1d_dev const &g2, std::array<int, 3> const &g,
                std::array<int, 3> const &dg, std::array<double, 3> const &h)
      : g2(g2), gx(g[0] + 1), gy(g[1] + 1), gx0(g[0]), gy0(g[1]), gz0(g[2]),
        dgx(dg[0]), dgy(dg[1]), dgz(dg[2]),
        padded_dgx(Distributed_fft3d::get_padded_shape_real(dgx)), hx(h[0]),
        hy(h[1]), hz(h[2]), igxgy(1.0 / (gx * gy)), igx(1.0 / gx), G000(0.0) {
...
struct alg_cplx_multiplier {
  ...
  alg_cplx_multiplier(karray1d_dev const &prod, karray1d_dev const &m1,
                      karray1d_dev const &m2, int offset)
...
struct alg_force_extractor {
  ...
  alg_force_extractor(karray1d_dev const &phi2, karray1d_dev const &enx,
                      ...
                      std::array<int, 3> const &g, std::array<int, 3> const &dg,
                      std::array<double, 3> const &h)
      : phi2(phi2), enx(enx), eny(eny), enz(enz), gx(g[0]), gy(g[1]), gz(g[2]),
        dgx(Distributed_fft3d::get_padded_shape_real(dg[0])), dgy(dg[1]),
...
struct alg_kicker {
  ...
  alg_kicker(Particles parts, ConstParticleMasks masks, karray1d_dev const &enx,
             karray1d_dev const &eny, karray1d_dev const &enz,
             std::array<int, 3> const &g, std::array<double, 3> const &h,
             std::array<double, 3> const &l, double factor, double pref,
             double m)
      : parts(parts), masks(masks), enx(enx), eny(eny), enz(enz), gx(g[0]),
        gy(g[1]), gz(g[2]), ihx(1.0 / h[0]), ihy(1.0 / h[1]), ihz(1.0 / h[2]),
        lx(l[0]), ly(l[1]), lz(l[2]), factor(factor), pref(pref), m(m) {}
...
Space_charge_3d_open_hockney::Space_charge_3d_open_hockney(
    ...
    : Collective_operator("sc_3d_open_hockney", 1.0), options(ops),
      bunch_sim_id(), domain(ops.shape, {1.0, 1.0, 1.0}),
      doubled_domain(ops.doubled_shape, {1.0, 1.0, 1.0}),
      use_fixed_domain(false), ffts() {}

void Space_charge_3d_open_hockney::apply_impl(Bunch_simulator &sim,
                                              double time_step,
                                              Logger &logger) {
...
void Space_charge_3d_open_hockney::apply_bunch(Bunch &bunch,
...
void Space_charge_3d_open_hockney::construct_workspaces(
...
void Space_charge_3d_open_hockney::update_domain(Bunch const &bunch) {
  scoped_simple_timer timer("sc3d_domain");

  // do nothing for fixed domain
  if (options.domain_fixed)
    return;
  ...
  doubled_domain = Rectangular_grid_domain(options.doubled_shape, doubled_size,
                                           offset, false);
}

void Space_charge_3d_open_hockney::set_fixed_domain(
    std::array<double, 3> offset, std::array<double, 3> size) {
  if (options.grid_entire_period) {
    offset[2] = 0.0;
    size[2] = options.z_period;
  }
...
void Space_charge_3d_open_hockney::get_local_charge_density(
    Bunch const &bunch) {
...
void Space_charge_3d_open_hockney::get_global_charge_density(
    Bunch const &bunch) {
  ...
  int err = MPI_Allreduce(MPI_IN_PLACE, (void *)h_rho2.data(), h_rho2.extent(0),
...
void Space_charge_3d_open_hockney::get_green_fn2_pointlike() {
...
void Space_charge_3d_open_hockney::get_green_fn2_linear() {
  if (options.periodic_z) {
    throw std::runtime_error(
...
void Space_charge_3d_open_hockney::get_local_phi2(Distributed_fft3d &fft) {
...
void Space_charge_3d_open_hockney::get_global_phi2(
  ...
  int err = MPI_Allreduce(MPI_IN_PLACE, (void *)h_phi2.data(),
                          nx_real * dg[1] * dg[2], MPI_DOUBLE, MPI_SUM,
                          fft.get_comm());
...
double Space_charge_3d_open_hockney::get_normalization_force(
    Distributed_fft3d const &fft) {
...
void Space_charge_3d_open_hockney::get_force() {
  auto g = domain.get_grid_shape();
  auto h = doubled_domain.get_cell_size();
  auto dg = doubled_domain.get_grid_shape();
...
void Space_charge_3d_open_hockney::apply_kick(Bunch &bunch, double fn_norm,
                                              double time_step) {
...
#pragma omp parallel for default(none), private(x, y, z, G, mix, miy, rr),     \
