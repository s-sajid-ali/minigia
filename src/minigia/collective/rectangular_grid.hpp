...
public:
  Rectangular_grid_1d(size_t shape_x, bool zero = true)
      : grid_(zero ? karray1d_row_dev("g1d", shape_x)
        ...
        hgrid_(Kokkos::create_mirror_view(grid_)), normalization(1.0) {}

  size_t shape(size_t dim) const { return grid_.extent(dim); }

  size_t span() const { return grid_.span(); }

  double *data(size_t x = 0) const { return &grid_(x); }
...
public:
  Rectangular_grid_2dc(size_t shape_x, size_t shape_y, bool zero = true)
      : grid_(zero
                  ? karray2dc_row_dev("g2dc", shape_x, shape_y)
