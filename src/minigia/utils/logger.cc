...
Logger::Logger(int rank, LoggerV verbosity, bool log)
    : stream_ptr(&std::cout), have_stream(false), fstream_ptr(0),
      have_fstream(false), verbosity(verbosity), severity(LoggerV::INFO) {
  if ((Commxx().get_rank() == rank) && log) {
    have_stream = true;
  }
}

Logger::Logger(int rank, std::string const &filename, LoggerV verbosity,
               bool screen, bool log)
    : stream_ptr(&std::cout), have_stream(false), fstream_ptr(0),
      have_fstream(false), verbosity(verbosity), severity(LoggerV::INFO) {
  if ((Commxx().get_rank() == rank) && log) {
    have_stream = screen;
...
Logger::Logger(int rank, char const *filename, LoggerV verbosity, bool screen,
               bool log)
    : stream_ptr(&std::cout), have_stream(false), fstream_ptr(0),
      have_fstream(false), verbosity(verbosity), severity(LoggerV::INFO) {
  if ((Commxx().get_rank() == rank) && log) {
...
Logger &Logger::set_stream(std::ostream &stream) {
  stream_ptr = &stream;
  return *this;
}

Logger &Logger::write(std::string const &str) { return (*this) << str; }
