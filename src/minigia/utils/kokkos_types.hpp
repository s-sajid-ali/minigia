...
namespace kt {
template <class T, size_t N> struct array_type {
  T data[N] = {};
...
// alias
template <class T, int N> using arr_t = array_type<T, N>;

template <typename T> KOKKOS_INLINE_FUNCTION T qpow(T x, int i) {
  T retval{1};
  while (i--) {
    retval = retval * x;
  }
...
template <class T, int N> struct SumArray {
public:
  // Required
