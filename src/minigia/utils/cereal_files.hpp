...
void archive_save(T const &object, std::string const &filename,
                  bool parallel = false) {
  int try_no = 0;
  bool fail = true;
  while ((try_no < 5) && fail) {
    ...
    // catch(boost::archive::archive_exception& be){
      ...
      std::cout << " boost archive exception  has been thrown: "
                << // be.what()<<
          "; trying again archive_save; trying number= " << try_no << std::endl
                << std::flush;
      sleep(3);
    }
  }
...
template <typename T, typename A>
void archive_load(T &object, std::string const &filename) {
...
template <typename T>
void binary_save(T const &object, std::string const &filename,
                 bool parallel = false) {
  archive_save<T, cereal::BinaryOutputArchive>(object, filename, parallel);
}

template <typename T> void binary_load(T &object, std::string const &filename) {
  archive_load<T, cereal::BinaryInputArchive>(object, filename);
}

template <typename T>
void xml_save(T const &object, std::string const &filename,
              bool parallel = false) {
  archive_save<T, cereal::XMLOutputArchive>(object, filename, parallel);
}

template <typename T> void xml_load(T &object, std::string const &filename) {
  archive_load<T, cereal::XMLInputArchive>(object, filename);
}

template <typename T>
void json_save(T const &object, std::string const &filename,
               bool parallel = false) {
  archive_save<T, cereal::JSONOutputArchive>(object, filename, parallel);
}

template <typename T> void json_load(T &object, std::string const &filename) {
