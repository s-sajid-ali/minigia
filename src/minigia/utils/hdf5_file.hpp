...
private:
  ...
  static unsigned int flag_to_h5_flags(Flag flag) {
    ...
    if (flag == Hdf5_file::Flag::read_write)
      return H5F_ACC_RDWR;
    if (flag == Hdf5_file::Flag::read_only)
...
public:
  ...
  std::string const &get_filename() const { return file_name; }
  ...
  hid_t get_h5file() { return h5file.hid; }

  int master_rank() const { return root_rank; }

  // gather on the first dimension. all other dimensions must be of the same
  // extents calling from 4 ranks:
  ...
  template <typename T> T read(std::string const &name) const {
  ...
  template <typename T> T read(std::string const &name, size_t len) const {
  ...
  std::vector<hsize_t> get_dims(std::string const &name) const {
  ...
  bool has_dataset(std::string const &name) const {
