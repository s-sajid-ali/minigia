...
public:
  Hdf5_seq_writer(Hdf5_handler const &file, std::string const &name,
                  Commxx const &comm, int root_rank)
      : file(file), name(name), comm(comm), root(root_rank),
        mpi_size(comm.size()), mpi_rank(comm.rank()), fdims(), offset(),
        dataset(), setup(false) {}

  template <typename T> void append(T const &data, bool collective) {
  ...
  bool verify_dims(syn::data_info_t const &di, hsize_t dim0) {
    if (fdims.size() > 1 && dim0 != fdims[1])
      return false;

    for (int i = 2; i < di.dims.size(); ++i)
      if (di.dims[i] != fdims[i])
        return false;

    return true;
  }

  void setup_dataset(syn::data_info_t &di, std::vector<hsize_t> const &offsets,
                     hsize_t dim0, bool has_dataset) {
    ...
    if (has_dataset) {
      ...
      if (dim0 == 0)
        throw std::runtime_error("Hdf5_seq_writer: zero data size encountered");
      ...
      dataset = H5Dcreate(file, name.c_str(), di.atomic_type, fspace,
                          H5P_DEFAULT, cparms, H5P_DEFAULT);
    }
  }

  void do_append(syn::data_info_t const &di,
    ...
    Hdf5_handler fspace = H5Dget_space(dataset);
    if (offset.size() > 1)
      offset[1] = offsets[mpi_rank];

    res = H5Sselect_hyperslab(fspace, H5S_SELECT_SET, offset.data(), NULL,
                              dimsm.data(), NULL);
    ...
    if (mpi_rank == root) {
      ...
      for (int r = 0; r < mpi_size; ++r) {
        ...
        if (r == root) {
          // write
          res = H5Dwrite(dataset, di.atomic_type, mspace, fspace, H5P_DEFAULT,
                         di.ptr);
          if (res < 0)
            throw Hdf5_exception();
        } else {
      ...
      if (sz)
        MPI_Send((void *)di.ptr, sz, MPI_BYTE, root, 0, comm);
    }
