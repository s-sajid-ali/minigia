...
public:
  ...
  //
  template <class T>
  static T read(Hdf5_handler const &file, std::string const &name,
                Commxx const &comm, int root_rank) {
    T retval;
    ...
    read_verify_and_set_data_dims(file, name, all_dim0, di, false, comm,
                                  root_rank);
  ...
  template <class T>
  static T read(Hdf5_handler const &file, std::string const &name, size_t len,
                Commxx const &comm, int root_rank) {
    T retval;
    auto di = syn::extract_data_info(retval);
    ...
    read_verify_and_set_data_dims(file, name, all_dim0, di, true, comm,
                                  root_rank);
  ...
  // read 1d array
  template <class T>
  static void read(Hdf5_handler const &file, std::string const &name, T *data,
                   Commxx const &comm, int root_rank) {
  ...
  template <class T>
  static void read(Hdf5_handler const &file, std::string const &name, T *data,
                   size_t len, Commxx const &comm, int root_rank) {
    syn::data_info_t di{
  ...
  static std::vector<hsize_t> get_dims(Hdf5_handler const &file,
  ...
  static void read_verify_and_set_data_dims(Hdf5_handler const &file,
                                            std::string const &name,
    ...
    MPI_Bcast(&error, 1, MPI_INT, root_rank, comm);
    if (error)
      throw std::runtime_error("err at root_rank");
  ...
  static void read_impl(Hdf5_handler const &file, std::string const &name,
                        std::vector<hsize_t> const &all_dims_0,
                        syn::data_info_t const &di, bool collective,
                        Commxx const &comm, int root_rank) {
    int mpi_size = comm.size();
    int mpi_rank = comm.rank();
    ...
    if (data_rank) {
      ...
      auto res = H5Sselect_hyperslab(fspace, H5S_SELECT_SET, offset.data(),
                                     NULL, di.dims.data(), NULL);
    ...
    // read
    auto res =
        H5Dread(dset, di.atomic_type, mspace, fspace, plist_id, (void *)di.ptr);
    if (res < 0)
      throw Hdf5_exception("error read");
    ...
    if (collective) {
      ...
      if (mpi_rank == root_rank) {
        ...
        for (int r = 0; r < mpi_size; ++r) {
          ...
          if (data_rank) {
            ...
            auto res =
                H5Sselect_hyperslab(fspace, H5S_SELECT_SET, offset.data(), NULL,
                                    dimsm.data(), NULL);
      ...
      if (mpi_rank == root_rank) {
        ...
        // read
        auto res = H5Dread(dset, di.atomic_type, mspace, fspace, H5P_DEFAULT,
                           (void *)di.ptr);
        if (res < 0)
          throw Hdf5_exception("error read");
      }
      ...
      // brodcast
      if (sz)
        MPI_Bcast((void *)di.ptr, sz, MPI_BYTE, root_rank, comm);
    }
#endif
  }

  static void bcast_vec_str_send(std::vector<std::string> const &vs,
                                 Commxx const &comm, int root) {
    size_t total = 0;
  ...
  static void bcast_vec_str_recv(std::vector<std::string> &vs,
                                 Commxx const &comm, int root) {
    size_t l = 0;
    MPI_Bcast(&l, 1, MPI_UINT64_T, root, comm);
  ...
  static std::vector<std::string> get_dataset_names(Hdf5_handler const &file,
                                                    Commxx const &comm,
                                                    int root_rank) {

    std::vector<std::string> names;

    auto cb = [](hid_t oid, const char *name, const H5O_info_t *info,
                 void *op) {
...
#else
    auto res = H5Ovisit(file, H5_INDEX_NAME, H5_ITER_NATIVE, cb, (void *)&names,
                        H5O_INFO_BASIC);
#endif
    ...
    if (comm.rank() == root_rank) {
      ...
      auto res =
          H5Ovisit(file, H5_INDEX_NAME, H5_ITER_NATIVE, cb, (void *)&names);
#else
      auto res = H5Ovisit(file, H5_INDEX_NAME, H5_ITER_NATIVE, cb,
                          (void *)&names, H5O_INFO_BASIC);
#endif
  ...
  static bool has_dataset(Hdf5_handler const &file, std::string const &name,
                          Commxx const &comm, int root_rank) {
    int has = 0;
...
#else
        auto res = H5Oget_info_by_name(file, name.c_str(), &info,
