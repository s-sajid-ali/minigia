...
Hdf5_file::Hdf5_file(std::string const &file_name, Flag flag, Commxx const &c)
    : comm(std::make_shared<Commxx>(c)), file_name(file_name), h5file(),
      root_rank(c.size() - 1), is_open(false), current_flag(flag)
...
Hdf5_file::Hdf5_file(std::string const &file_name, Flag flag,
    ...
    : comm(c), file_name(file_name), h5file(), root_rank(c->size() - 1),
      is_open(false), current_flag(flag)
#ifdef USE_PARALLEL_HDF5
      ,
      has_file(true)
#else
  ...
  while ((attempts < 5) && fail) {
    try {
      ...
      if (flag == Hdf5_file::Flag::truncate) {
        // create
        h5file =
            H5Fcreate(file_name.c_str(), H5F_ACC_TRUNC, H5P_DEFAULT, plist_id);
