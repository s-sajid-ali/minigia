...
private:
  ...
  template <typename T> std::vector<hsize_t> pre_setup(T const &data) {
    data_rank = T::Rank;
  ...
  // default for karray types
  template <typename T> void *get_data_ptr(T const &data) {
    return (void *)data.data();
  }

public:
  Hdf5_serial_writer(hid_t file_ptr, std::string const &name)
      : data_rank(0), name(name), dims(), max_dims(), size(), offset(),
        file_ptr(file_ptr), dataset(), atomic_type(), have_setup(false),
        data_size(0) {}

  template <typename T> void append(T const &data) {
    if (!have_setup)
      do_setup(pre_setup(data));
