...
TEST_CASE("default construct", "[commxx]") { REQUIRE_NOTHROW(Commxx()); }
...
TEST_CASE("split with color only") {
  ...
  SECTION("world") {
    ...
    SECTION("no throw") { REQUIRE_NOTHROW(comm1_ptr->split(0)); }
...
TEST_CASE("split with color and key") {
  ...
  SECTION("world") {
    ...
    SECTION("no throw") { REQUIRE_NOTHROW(comm1_ptr->split(0, world_rank)); }
...
TEST_CASE("divide") {
  ...
  SECTION("world") {
    ...
    if (world_size == 2) {
      ...
      SECTION("uneven divide") { REQUIRE_THROWS(comm1_ptr->divide(2)); }
    } else if (world_size == 4) {
      SECTION("divide into 2 groups") {
        int subgroup_size = world_size / 2;
