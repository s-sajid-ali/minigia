...
std::string Lsexpr::to_string(int i) const {
  std::stringstream ss;
  ss << i;
  return ss.str();
}

int Lsexpr::stoi(std::string const &s) const {
  int retval;
...
std::string Lsexpr::to_string(double d) const {
  std::stringstream ss;
...
double Lsexpr::stod(std::string const &s) const {
  double retval;
  std::istringstream ss(s);
  ss >> retval;
  return retval;
...
void Lsexpr::set_label(std::string const &label) { this->label = label; }

bool Lsexpr::is_labeled() const { return !label.empty(); }

std::string const &Lsexpr::get_label() const { return label; }

bool Lsexpr::is_atomic() const { return is_atom; }

std::string Lsexpr::get_string() const {
  if (!is_atom) {
...
double Lsexpr::get_double() const {
  if (!is_atom) {
...
void Lsexpr::push_back(Lsexpr const &lsexpr) { sequence.push_back(lsexpr); }

Lsexpr::const_iterator_t Lsexpr::begin() const { return sequence.begin(); }

Lsexpr::const_iterator_t Lsexpr::end() const { return sequence.end(); }

Lsexpr::iterator_t Lsexpr::begin() { return sequence.begin(); }

Lsexpr::iterator_t Lsexpr::end() { return sequence.end(); }

std::vector<std::string> Lsexpr::get_string_vector() const {
  if (is_atom) {
...
std::vector<int> Lsexpr::get_int_vector() const {
  if (is_atom) {
...
std::vector<double> Lsexpr::get_double_vector() const {
  if (is_atom) {
...
void Lsexpr::write(std::ostream &stream) const {
...
Lsexpr read_lsexpr_file(std::string const &filename) {
  ...
  if (!f.is_open()) {
    throw std::runtime_error("read_lsexpr_file:: unable to open '" + filename +
                             "'");
  }
  Lsexpr retval(f);
  return retval;
...
void write_lsexpr_file(Lsexpr const &lsexpr, std::string const &filename) {
  ...
  if (!f.is_open()) {
    throw std::runtime_error("write_lsexpr_file:: unable to open '" + filename +
                             "'");
  }
  lsexpr.write(f);
}
