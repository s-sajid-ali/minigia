...
namespace {
bool same_nth_dim(std::vector<hsize_t> const &dims, size_t dim, size_t ndim,
                  size_t mpi_size) {
  assert(mpi_size > 0);
...
bool same_data_ranks(std::vector<size_t> const &ranks) {
  auto it =
      std::adjacent_find(ranks.begin(), ranks.end(), std::not_equal_to<>());
  return it == ranks.end();
}
} // namespace

std::vector<hsize_t> syn::collect_dims(std::vector<hsize_t> const &dims,
  ...
  if (collective && !dims.size()) {
    throw std::runtime_error("Hdf5_writer: collective read/write on a scalar, "
  ...
  MPI_Allgather(&data_rank, 1, MPI_UINT64_T, all_ranks.data(), 1, MPI_UINT64_T,
                comm);
  ...
  MPI_Allgather(local_dims.data(), data_rank, MPI_UINT64_T, all_dims.data(),
                data_rank, MPI_UINT64_T, comm);
  ...
  for (int d = d0; d < data_rank; ++d) {
    if (!same_nth_dim(all_dims, d, data_rank, mpi_size))
      throw std::runtime_error("Hdf5_writer: inconsistent data dimensions");
  }
