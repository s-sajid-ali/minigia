...
private:
  static herr_t err_walk(unsigned int n, H5E_error_t const *err_desc,
                         void *client_data) {
    ...
    (*ss) << "  #" << std::setfill('0') << std::setw(4) << n << ": "
          << err_desc->file_name << " line " << err_desc->line << " in "
          << err_desc->func_name << "(): " << err_desc->desc << "\n"
          << "    major(" << std::setfill('0') << std::setw(3)
          << (int)err_desc->maj_num << "): " << maj_str << "\n"
          << "    minor(" << std::setfill('0') << std::setw(3)
          << (int)err_desc->min_num << "): " << min_str << "\n";
...
struct Hdf5_handler {
  ...
  Hdf5_handler(Hdf5_handler &&o) noexcept : hid(o.hid) { o.hid = 0; }
  ...
  void close() {
    ...
    case H5I_GENPROP_LST:
      ...
      break;
    default:
      throw std::runtime_error("close of unhandled Hdf5 handler type");
    }
...
template <typename T> inline hid_t hdf5_atomic_data_type() {
...
template <> inline hid_t hdf5_atomic_data_type<uint8_t>() {
  return H5Tcopy(H5T_NATIVE_UCHAR);
}

template <> inline hid_t hdf5_atomic_data_type<int>() {
  return H5Tcopy(H5T_NATIVE_INT);
}

template <> inline hid_t hdf5_atomic_data_type<double>() {
...
resize_data_obj(T &t, data_info_t &di) {
  if (di.dims.size())
    throw std::runtime_error("resize_data_obj: non-zero rank resizing scalar");
}
...
resize_data_obj(T &t, data_info_t &di) {
  if (di.dims.size() != T::Rank)
    throw std::runtime_error("resize_data_obj: inconsistent data rank");

  switch (di.dims.size()) {
  case 1:
    Kokkos::resize(t, di.dims[0]);
    break;
  case 2:
    Kokkos::resize(t, di.dims[0], di.dims[1]);
    break;
  case 3:
    Kokkos::resize(t, di.dims[0], di.dims[1], di.dims[2]);
    break;
  default:
...
//----+-----------+-----------+-------+-----------+----------------------
//
std::vector<hsize_t> collect_dims(std::vector<hsize_t> const &dims,
