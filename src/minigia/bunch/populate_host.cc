...
void adjust_moments_host(double const *means, double const *covariances,
                         double const *bunch_mean, double const *bunch_mom2,
                         int num_particles, int num_particles_slots,
                         double *particles) {
...
void get_correlation_matrix_host(double *correlation_matrix,
                                 double const *one_turn_map, double arms,
                                 double brms, double crms, double beta,
  ...
  for (int i = 0; i < 6; i++) {
    for (int j = 0; j < 6; j++) {
      ...
      for (int k = 0; k < 3; k++)
        c_matrix(i, j) += F[k](i, j) * (Sinv(k, 0) * cd1 + Sinv(k, 1) * cd2 +
                                        Sinv(k, 2) * cd3);
    }
  }
