...
struct discarded_particle_mover {
  ...
  discarded_particle_mover(ConstParticles const &parts,
      ...
      : counter("counter", 1), parts(parts), discards(discards),
...
template <> void bunch_particles_t<double>::reserve_local(int r) {
  if (r <= n_reserved)
    return;
...
template <> void bunch_particles_t<double>::reserve(int n, Commxx const &comm) {
...
template <>
void bunch_particles_t<double>::inject(bunch_particles_t const &o,
  ...
  // inject
  particle_injector pi{parts,  masks,  o.parts,  o.masks, ref_st_diff,
                       tgt_st, inj_st, n_active, pdiff};
...
template <>
void bunch_particles_t<double>::convert_to_fixed_t_lab(double p_ref,
                                                       double beta) {
...
template <>
void bunch_particles_t<double>::convert_to_fixed_z_lab(double p_ref,
                                                       double beta) {
...
template <>
int bunch_particles_t<double>::search_particle(int pid, int last_idx) const {
...
template <>
karray2d_row bunch_particles_t<double>::get_particles_last_discarded() const {
...
template <> int bunch_particles_t<double>::update_valid_num() {
...
template <>
int bunch_particles_t<double>::update_total_num(Commxx const &comm) {
  int old_total_num = n_total;
  MPI_Allreduce(&n_valid, &n_total, 1, MPI_INT, MPI_SUM, comm);
  return old_total_num;
}

template <> void bunch_particles_t<double>::check_pz2_positive() {
  checkout_particles();
  ...
  for (int p = 0; p < n_active; ++p) {
    ...
    double pzop2 = (1. + hparts(p, 5)) * (1. + hparts(p, 5)) -
                   hparts(p, 1) * hparts(p, 1) - hparts(p, 3) * hparts(p, 3);
...
template <>
void bunch_particles_t<double>::read_file_legacy(Hdf5_file const &file,
                                                 Commxx const &comm) {
...
template <>
void bunch_particles_t<double>::read_file(Hdf5_file const &file,
                                          Commxx const &comm) {
...
template <>
void bunch_particles_t<double>::write_file(Hdf5_file const &file, int num_part,
                                           int offset,
...
template <>
void bunch_particles_t<double>::print_particle(size_t idx,
                                               Logger &logger) const {
...
template <>
void bunch_particles_t<double>::save_checkpoint_particles(Hdf5_file &file,
                                                          int idx) const {
  checkout_particles();
...
template <>
void bunch_particles_t<double>::load_checkpoint_particles(Hdf5_file &file,
                                                          int idx) {
  std::stringstream ss;
  ss << "bunch_particles_" << label << "_parts_" << idx;
