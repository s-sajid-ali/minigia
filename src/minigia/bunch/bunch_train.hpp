...
class Bunch_train {
  ...
  private:
    ...
    /// counts and offsets are needed for impedance,
    /// counts.size()=offsets.size()=num_procs they are meaningfull only on the
    /// local rank=0 of every bunch communicator for example:  3 bunches on 5
    /// processors: proc 2 and 4 has local rank different form zero
    ///              (count[0]=1, offset[0]=0), (count[1]=1, offset[3]=1),
    ///              (count[3]=1, offset[3]=2)
    /// for example:  5 bunches on 3 processors:
    ///                (count[0]=1, offset[0]=0), (count[1]=2, offset[3]=1),
    ///                (count[2]=2, offset[2]=3)
  ...
  public:
    ...
    Bunch_train(Reference_particle const &ref, size_t num_bunches,
        size_t num_particles_per_bunch,
        double num_real_particles_per_bunch, double spacing,
        Commxx const &comm = Commxx(), size_t num_spectator_per_bunch = 0,
        int index = 0);
    ...
    int get_num_bunches() const { return num_bunches; }

    // number of local bunches on this rank. alias for
    // get_bunch_array_size()
    int get_num_local_bunches() const { return get_bunch_array_size(); }
    ...
    size_t get_bunch_array_size() const { return bunches.size(); }
    ...
    Bunch &operator[](size_t array_idx) { return bunches[array_idx]; }

    Bunch const &operator[](size_t array_idx) const { return bunches[array_idx]; }

    // access the local bunch array
    std::vector<Bunch> &get_bunches() { return bunches; }

    std::vector<Bunch> const &get_bunches() const { return bunches; }
