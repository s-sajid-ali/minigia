...
#include <minigia/utils/restrict_extension.hpp>

#include "bunch_particles.hpp"
#include "diagnostics_loss.hpp"
#include "diagnostics_worker.hpp"
...
template <class PART> class bunch_t {
  public:
  ...
  public:
    ...
    bunch_t(Reference_particle const &reference_particle, int total_num,
        double real_num, Commxx comm = Commxx(), int total_spectator_num = 0,
        int bunch_index = 0, int bucket_index = 0, int array_index = 0,
        int train_index = 0);

    // to construct a bunch with trigon particles
    template <typename U = PART>
      bunch_t(Reference_particle const &reference_particle, int total_num = 1,
          Commxx comm = Commxx(),
    ...
    // bunch particles
    bunch_particles_t<PART> &get_bunch_particles(ParticleGroup pg = PG::regular) {
      return parts[(int)pg];
    }
    ...
    masks_t get_local_particle_masks(ParticleGroup pg = PG::regular) {
      return get_bunch_particles(pg).masks;
    }

    const_masks_t get_local_particle_masks(ParticleGroup pg = PG::regular) const {
      return get_bunch_particles(pg).masks;
    }

    // get the host particle arrays and masks arrays
    host_parts_t get_host_particles(ParticleGroup pg = PG::regular) {
      return get_bunch_particles(pg).hparts;
    }

    const_host_parts_t get_host_particles(ParticleGroup pg = PG::regular) const {
      return get_bunch_particles(pg).hparts;
    }

    host_masks_t get_host_particle_masks(ParticleGroup pg = PG::regular) {
    ...
    karray2d_row get_particles_in_range(int idx, int num,
    ...
    // Diagnostics
    template <class Diag>
      std::pair<Diagnostics_handler, int> add_diagnostics(Diag const &diag) {
    ...
    std::string diag_type(int id) const { return diags[id].type(); }

    void diag_update(int id) { get_diag(id).update(); }

    void diag_update_and_write(int id) { get_diag(id).update_and_write(); }
    ...
    void set_diag_loss_zcut(std::string const &filename) {
      diag_zcut.reset(new Diagnostics_worker(Diagnostics_loss(filename), comm));
    }
    ...
    void write_file(std::string const &filename, int num_part = -1,
        int offset = 0, int num_part_spec = 0,
    ...
    void write_file(Hdf5_file const &file, int num_part, int offset,
...
inline bunch_t<PART>::bunch_t(
    Reference_particle const &reference_particle, int total_num,
    Commxx bunch_comm, typename std::enable_if<is_trigon<U>::value>::type *)
  : comm(std::make_shared<Commxx>(bunch_comm)), boundary(LB::open),
  boundary_param(0.0), ref_part(reference_particle),
  design_ref_part(reference_particle),
  particle_charge(reference_particle.get_charge()),
  real_num(1.0), parts{bunch_particles_t<PART>(PG::regular, total_num, -1,
      *comm),
  bunch_particles_t<PART>(PG::spectator, 0, -1,
      *comm)},
  bunch_index(0), bucket_index(0), array_index(0), train_index(0) {}
