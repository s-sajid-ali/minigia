...
template <>
Bunch::bunch_t(Reference_particle const &reference_particle, int total_num,
               double real_num, Commxx bunch_comm, int total_spectator_num,
               int bunch_index, int bucket_index, int array_index,
               int train_index)
    : comm(std::make_shared<Commxx>(bunch_comm)), boundary(LB::open),
      ...
      real_num(real_num), parts{BunchParticles(PG::regular, total_num, -1,
                                               *comm),
                                BunchParticles(PG::spectator,
                                               total_spectator_num, -1, *comm)},
      bunch_index(bunch_index), bucket_index(bucket_index),
      array_index(array_index), train_index(train_index) {}

template <>
Bunch::bunch_t()
    : comm(new Commxx()), boundary(LB::open), boundary_param(0.0), ref_part(),
      design_ref_part(), particle_charge(ref_part.get_charge()),
      real_num(1.0), parts{BunchParticles(PG::regular, 0, 0, *comm),
                           BunchParticles(PG::spectator, 0, 0, *comm)},
      bunch_index(0), bucket_index(0), array_index(0), train_index(0) {}

template <> void Bunch::inject(Bunch const &o) {
  ...
  if (total_num != 0) {
    ...
    if (std::abs(wgt1 / wgt2 - 1.0) > weight_tolerance) {
      throw std::runtime_error("Bunch.inject: macroparticle weight of injected "
                               "bunch does not match.");
    }
  }
  ...
  get_bunch_particles(PG::regular)
      .inject(o.get_bunch_particles(PG::regular), ref_st_diff, tgt_st, inj_st,
              pdiff);

  // spectator
  get_bunch_particles(PG::spectator)
      .inject(o.get_bunch_particles(PG::spectator), ref_st_diff, tgt_st, inj_st,
              pdiff);
  ...
  if (old_total == 0)
    real_num = o.get_real_num();
}

template <> void Bunch::print_statistics(Logger &logger) const {
  using LV = LoggerV;
