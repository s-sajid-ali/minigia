...
void Bunch_train::calculates_counts_and_offsets_for_impedance() {
#if 0
...
void Bunch_train::set_bucket_indices() {
#if 0
...
Bunch_train::Bunch_train(Reference_particle const &ref, size_t num_bunches,
                         size_t num_particles_per_bunch,
                         double num_real_particles_per_bunch, double spacing,
                         Commxx const &bt_comm, size_t num_spectator_per_bunch,
                         int index)
    : comm(std::make_shared<Commxx>(bt_comm)), bunches(), spacings(),
      index(index), num_bunches(num_bunches), num_buckets(num_bunches),
  ...
  for (int b = 0; b < bunches_per_rank; ++b) {
    ...
    bunches.emplace_back(ref, num_particles_per_bunch,
                         num_real_particles_per_bunch, comm->split(color),
...
std::vector<double> &Bunch_train::get_spacings() { return spacings; }

std::vector<int> &Bunch_train::get_proc_counts_for_impedance() {
  return proc_counts_imped;
}

std::vector<int> &Bunch_train::get_proc_offsets_for_impedance() {
  return proc_offsets_imped;
}

void Bunch_train::update_bunch_total_num() {
#if 0
