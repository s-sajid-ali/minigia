...
public:
  Diagnostics(std::string const &type = "Diagnostics",
              std::string const &filename = "diag.h5", bool single_file = true)
  ...
  void update(Bunch const &bunch) { do_update(bunch); }

  void reduce(Commxx const &comm, int root) { do_reduce(comm, root); }
