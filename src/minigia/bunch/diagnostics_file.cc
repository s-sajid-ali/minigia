...
#include "diagnostics_file.hpp"

#pragma message "TODO: replace boost::filesystem here"

std::string Diagnostics_file::temp_dir = "";

Diagnostics_file::Diagnostics_file(std::string const &filename, bool serial,
                                   std::shared_ptr<Commxx> const &comm)
    : file(), serial(serial), file_count(0), filename(filename),
      filename_base(), filename_suffix(), filename_appendix() {
  ...
  file = std::make_unique<Hdf5_file>(get_filename(true),
                                     Hdf5_file::Flag::truncate, comm);
}

Diagnostics_file::Diagnostics_file()
    : file(), serial(true), file_count(0), filename(), filename_base(),
      filename_suffix(), filename_appendix() {}

Diagnostics_file::~Diagnostics_file() { close_file(); }
...
void Diagnostics_file::move_file_overwrite_if_exists(std::string const &src,
