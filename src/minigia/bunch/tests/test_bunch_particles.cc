...
TEST_CASE("BunchParticles", "[BunchParticles]") {
  ...
  SECTION("verify values") { check_particle_values(bp); }
  ...
  SECTION("write/read file") {
    ...
    SECTION("read into 0 size but reserved bunch particle") {
      BunchParticles bp2(ParticleGroup::regular, 0, np + 6, Commxx::World);
    ...
    SECTION("read into non-zero sized bunch particle") {
      BunchParticles bp2(ParticleGroup::regular, np + 6, -1, Commxx::World);
