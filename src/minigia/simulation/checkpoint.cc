...
namespace syn {
void checkpoint_save_as_json(std::string const &prop_str,
...
void syn::checkpoint_save(Propagator const &prop, Bunch_simulator const &sim) {
  ...
  // gather strings
  MPI_Gatherv(sim_str.data(), sim_str.size(), MPI_CHAR, (void *)sims_str.data(),
              lens.data(), displs.data(), MPI_CHAR, root, comm);
...
std::pair<Propagator, Bunch_simulator> syn::checkpoint_load() {
  const int root = 0;
  ...
  if (mpi_rank == root) {
    ...
    if (!file.good())
      throw std::runtime_error("Error at openning checkpointing file");
  ...
  return std::make_pair(Propagator::load_from_string(cp.first),
