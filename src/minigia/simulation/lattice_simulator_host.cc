...
Vector ev_ordering(Vector const &ev, Matrix const &B);

std::array<double, 2>
Lattice_simulator::filter_transverse_tunes(double const *jac_arr) {
  ...
  if (jac(iy, ix) || jac(ix, iy) || jac(ix, ipy) || jac(iy, ipx) ||
      jac(ipy, ix) || jac(ipx, iy) || jac(ipy, ipx) || jac(ipx, ipy)) {
    ...
    for (int i = 0; i < 6; ++i) {
      if (fabs(abs(lambda(i)) - 1.0) > 1.0e-4) {
        ...
        ss << "filterTransverseTunes: "
           ...
           << "lambda( " << i << " ) has magnitude = " << abs(lambda(i));
    ...
    MatrixD JH = A - csH * U;
    double snH =
        (JH(0, 1) > 0.0) ? sqrt(1.0 - csH * csH) : -sqrt(1.0 - csH * csH);

    // .......... A little test to keep everyone honest .....
    if (JH(0, 0)) {
      if (fabs((JH(0, 0) + JH(1, 1)) / (JH(0, 0) - JH(1, 1))) > 1.0e-4) {
        std::cout << "WARNING -- filterTransverseTunes: "
    ...
    MatrixD JV = B - csV * U;
    double snV =
        (JV(0, 1) > 0.0) ? sqrt(1.0 - csV * csV) : -sqrt(1.0 - csV * csV);
