#include <minigia/utils/string_utils.hpp>

#include "split_operator_stepper.hpp"
...
get_fixed_step_slices(ITER &it, ITER const &end, double &left,
                      double step_length, double &offset_fudge,
...
template <typename ITER>
void create_substep(std::vector<Step> &steps,
                    std::vector<std::shared_ptr<Operator>> const &col_op_ptrs,
                    ITER &it, ITER const &end, double left, double length,
                    double &offset_fudge) {
  // create the substep
  steps.emplace_back(length);

  if (length == 0.0) {
    auto slices = get_fixed_step_slices(it, end, left, 0.0, offset_fudge, true);
    ...
    for (auto const &s : fst_half_slices)
      op1.append_slice(s);
    ...
    for (auto const &s : snd_half_slices)
      op2.append_slice(s);
  }
}
} // namespace
...
Split_operator_stepper::apply_impl(Lattice const &lattice) const {
  ...
  for (int i = 0; i < num_steps; ++i) {
    ...
    all_slices.insert(all_slices.end(), fst_half_slices.begin(),
                      fst_half_slices.end());
    all_slices.insert(all_slices.end(), snd_half_slices.begin(),
                      snd_half_slices.end());
    ...
    for (int it = 0; it < all_slices.size() - 1; ++it) {
      ...
      if (s.has_right_edge() && ele.has_string_attribute(force_diag_attr) &&
        ...
        // create the substep
        create_substep(steps, col_op_ptrs, substep_lattice_it, lattice_end,
