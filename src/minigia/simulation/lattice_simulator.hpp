...
#include <minigia/foundation/normal_form.hpp>
...
// final coordinates. cdt is the c * the total propagation time of the particle
// which gives the total path length.

// find the closed orbit for the circular lattice, propagate the lattice
// reference particle through the lattice slices using the closed orbit, and set
// the reference c*t for each lattice slice after tuning. return values is the
// state for calcualted closed orbit note that all the rf cavities will be set
// to 0 strength during the tuning process
...
chromaticities_t get_chromaticities(Lattice const &lattice, double dpp = 1e-5);
...
template <class ELMS> void calc_dispersions_impl(Lattice &lattice, ELMS &elms);

// adjust tunes and chromaticities
void adjust_tunes(Lattice &lattice, double horizontal_tune,
                  double vertical_tune, double tolerance = 1e-5);

void adjust_chromaticities(Lattice &lattice, double horizontal_chromaticity,
                           double vertical_chromaticity,
                           double tolerance = 1e-4, int max_steps = 6);
...
void CourantSnyderLatticeFunctions_impl(Lattice &lattice, ELMS &elms) {
  ...
  double sn = (jac(ix, ipx) > 0.0) ? sqrt(1.0 - cs * cs) : -sqrt(1.0 - cs * cs);
