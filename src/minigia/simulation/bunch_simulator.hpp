...
#include <minigia/bunch/bunch_train.hpp>
#include <minigia/lattice/lattice.hpp>

#include "propagate_actions.hpp"
...
private:
  ...
  struct trigger_step_period {
    ...
    bool operator()(int turn, int step) const {
      ...
      if (step_period < 0)
        return (turn % turn_period == 0) && (step == FINAL_STEP);

      // at both the right turn and step
      return (turn % turn_period == 0) && (step % step_period == 0);
    }

    template <class AR> void serialize(AR &ar) { ar(turn_period, step_period); }
  };
  ...
  struct trigger_turn_listed {
    ...
    template <class AR> void serialize(AR &ar) { ar(numbers); }
  };
  ...
  struct trigger_element {
    ...
    bool operator()(int turn, Lattice_element const &ele) const {
      return (turn % turn_period == 0) && (ele.get_name() == name);
    }

    template <class AR> void serialize(AR &ar) { ar(name, turn_period); }
  };

  template <typename TriggerT> struct diag_tuple_t {
    int train;
    int bunch;
    int diag_id;
    TriggerT trigger;
  ...
  using action_data_step_t =
      std::function<void(Bunch_simulator &, Lattice &, int, int, void *)>;

  // void action(Bunch_simulator&, Lattice&, int turn, void* data)
  using action_turn_t = std::function<void(Bunch_simulator &, Lattice &, int)>;
  using action_data_turn_t =
      std::function<void(Bunch_simulator &, Lattice &, int, void *)>;
...
private:
  ...
  static Bunch_simulator construct(Reference_particle const &ref_pri,
                                   Reference_particle const &ref_sec,
                                   size_t num_part, size_t num_spec,
                                   double num_real_part, size_t num_bunches_pri,
                                   size_t num_bunches_sec, double spacing_pri,
...
public:
  ...
  // factory methods
  static Bunch_simulator create_empty_bunch_simulator();

  static Bunch_simulator
  create_single_bunch_simulator(Reference_particle const &ref,
                                size_t num_particles, double num_real_particles,
                                Commxx const &comm = Commxx(),
                                size_t num_spectators = 0);

  static Bunch_simulator create_bunch_train_simulator(
      Reference_particle const &ref, size_t num_particles,
      double num_real_particles, size_t num_bunches, double spacing,
      Commxx const &comm = Commxx(), size_t num_spectators = 0);

  static Bunch_simulator create_two_trains_simulator(
  ...
  // bunch simulator id
  std::string const &id() const { return uuid; }

  // register a per-turn diagnostics (with an optional turn period)
  // the train and bunch are indexed based on actual number of
  // bunches per train
  template <class Diag>
  Diagnostics_handler reg_diag_period(Diag const &diag, int turn_period,
                                      int step_period, int bunch, int train) {
    ...
    auto handler = trains[train][bunch_idx].add_diagnostics(diag);
  ...
  template <class Diag>
  Diagnostics_handler reg_diag_per_turn(Diag const &diag, int period = 1,
                                        int bunch = 0, int train = 0) {
    return reg_diag_period(diag, period, -1, bunch, train);
  }

  template <class Diag>
  Diagnostics_handler reg_diag_per_step(Diag const &diag, int period = 1,
                                        int bunch = 0, int train = 0) {
  ...
  template <class Diag>
  Diagnostics_handler reg_diag_turn_listed(Diag const &diag,
  ...
  template <class Diag>
  Diagnostics_handler
  reg_diag_at_element(Diag const &diag, Lattice_element const &ele,
                      int turn_period = 1, int bunch = 0, int train = 0) {
    if (turn_period <= 0)
  ...
  // diag loss
  void reg_diag_loss_aperture(std::string const &filename, int bunch = 0,
                              int train = 0) {
    get_bunch(train, bunch).set_diag_loss_aperture(filename);
  }

  void reg_diag_loss_zcut(std::string const &filename, int bunch = 0,
                          int train = 0) {
  ...
  template <class PA> void reg_prop_actions(PA const &pa) {
  ...
  void set_longitudinal_boundary(LongitudinalBoundary lb, double param = 0.0) {
  ...
  void populate_6d(uint64_t seed, const_karray1d means,
                   const_karray2d_row covariances);

  void populate_6d_truncated(uint64_t seed, const_karray1d means,
                             const_karray2d_row covariances,
...
template <>
inline void Bunch_simulator::reg_prop_actions(
    std::shared_ptr<Propagate_actions> const &pa) {
  prop_actions = pa;
}
