...
bool Independent_operator::need_update(Reference_particle const &ref,
                                       Logger &logger) {
  return false;
}

void Independent_operator::update_operations(
    Reference_particle const &reference_particle) {}

void Independent_operator::create_operations_impl(Lattice const &lattice) {
  operations.clear();
  ...
  for (auto const &slice : slices) {
    ...
    if (((extractor_type != last_extractor_type) || need_left_aperture) &&
        (!group.empty())) {
      extract_independent_operations(extractor_type, lattice, group,
                                     operations);
      group.clear();
    }
...
void Independent_operator::apply_impl(Bunch_simulator &simulator,
