...
void chef_map_operation_extract(
    Lattice const &lattice, std::vector<Lattice_element_slice> const &slices,
    std::vector<std::unique_ptr<Independent_operation>> &operations) {}

void chef_propagator_operation_extract(
    Lattice const &lattice, std::vector<Lattice_element_slice> const &slices,
    std::vector<std::unique_ptr<Independent_operation>> &operations) {}

void libff_operation_extract(
    Lattice const &lattice, std::vector<Lattice_element_slice> const &slices,
    std::vector<std::unique_ptr<Independent_operation>> &operations) {
...
void extract_independent_operations(
    std::string const &extractor_type, Lattice const &lattice,
