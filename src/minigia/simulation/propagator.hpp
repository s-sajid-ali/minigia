...
#include <minigia/utils/cereal.hpp>
...
public:
  ...
  struct Slice_iterator {
    ...
    void end() { step_it = prop.steps.end(); }
    ...
    friend bool operator==(const Slice_iterator &a, const Slice_iterator &b) {
      if (&a.prop != &b.prop)
        return false;
...
private:
  ...
  void do_turn_end(Bunch_simulator &simulator, int turn_count, Logger &logger);

  void do_step(Bunch_simulator &simulator, Step &step, int step_count,
               int turn_count, Logger &logger);

  bool check_out_of_particles(Bunch_simulator const &simulator, Logger &logger);

public:
  // given lattice and stepper
  Propagator(Lattice const &lattice,
             Stepper const &stepper = Independent_stepper_elements(1))
      : lattice(lattice), steps(), slices(*this), stepper_ptr(stepper.clone()),
        checkpoint_period(-1), final_checkpoint(false) {
    this->lattice.update();
  ...
  void set_checkpoint_period(int period) { checkpoint_period = period; }

  int get_checkpoint_period() const { return checkpoint_period; }

  // whether to perform checkpoint save at the end of simulation
  void set_final_checkpoint(bool val) { final_checkpoint = val; }

  bool get_final_checkpoint() const { return final_checkpoint; }

  // slices
  Lattice_element_slices &get_lattice_element_slices() { return slices; }

  // elements
  std::list<Lattice_element> const &get_lattice_elements() {
