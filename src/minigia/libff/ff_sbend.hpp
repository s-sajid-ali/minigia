...
template <class BP> struct PropSbend {
  ...
  void operator()(const int i) const {
    if (masks(i)) {
      if (sp.ledge) {
        // slot
        FF_algorithm::slot_unit(p(i, 0), p(i, 1), p(i, 2), p(i, 3), p(i, 4),
                                p(i, 5), sp.ce1, sp.se1, sp.pref_b, sp.m_b);
        ...
        FF_algorithm::edge_unit(p(i, 2), p(i, 1), p(i, 3), sp.us_edge_k_x,
                                sp.us_edge_k_y, 0);
      }

      // bend
      FF_algorithm::bend_unit(p(i, 0), p(i, 1), p(i, 2), p(i, 3), p(i, 4),
                              p(i, 5), dphi, sp.strength, sp.pref_b, sp.m_b,
                              sp.ref_cdt, phase, term);

      if (sp.redge) {
        // edge
        // FF_algorithm::edge_unit(y, yp, ds_edge_k);
        // FF_algorithm::edge_unit(y, xp, yp, dpop, ds_edge_k_p);
        FF_algorithm::edge_unit(p(i, 2), p(i, 1), p(i, 3), sp.ds_edge_k_x,
                                sp.ds_edge_k_y, 0);

        // slot
        FF_algorithm::slot_unit(p(i, 0), p(i, 1), p(i, 2), p(i, 3), p(i, 4),
                                p(i, 5), sp.ce2, sp.se2, sp.pref_b, sp.m_b);
      }
    }
  }
};

template <class BP> struct PropSbendSimd {
  ...
  void operator()(const int idx) const {
    ...
    if (m) {
      ...
      if (sp.ledge) {
        // slot
        FF_algorithm::slot_unit<gsv_t>(p0, p1, p2, p3, p4, p5, sp.ce1, sp.se1,
                                       sp.pref_b, sp.m_b);
        ...
        //
        // 3. ref particle angle (kicks both xp and yp)
        FF_algorithm::edge_unit<gsv_t>(p2, p1, p3, sp.us_edge_k_x,
                                       sp.us_edge_k_y, 0);
      }

      // bend
      FF_algorithm::bend_unit<gsv_t>(p0, p1, p2, p3, p4, p5, dphi, sp.strength,
                                     sp.pref_b, sp.m_b, sp.ref_cdt, phase,
                                     term);
      ...
      if (sp.redge) {
        ...
        FF_algorithm::edge_unit<gsv_t>(p2, p1, p3, sp.ds_edge_k_x,
                                       sp.ds_edge_k_y, 0);

        // slot
        FF_algorithm::slot_unit<gsv_t>(p0, p1, p2, p3, p4, p5, sp.ce2, sp.se2,
                                       sp.pref_b, sp.m_b);
      }
...
template <class BP> struct PropSbendCF {
  using gsv_t = typename BP::gsv_t;
  using parts_t = typename BP::parts_t;
  ...
  PropSbendCF(BP &bp, SbendParams const &sp)
      : p(bp.parts), masks(bp.masks), sp(sp)
        ...
        ,
        step_angle(sp.angle / sp.steps), step_ref_cdt(sp.ref_cdt / sp.steps)
        ...
        ,
        step_dphi{fa::sbend_dphi(by6::c1, step_angle),
  ...
  void operator()(const int i) const {
    if (masks(i)) {
      if (sp.ledge) {
        ...
        FF_algorithm::bend_edge(p(i, 0), p(i, 1), p(i, 2), p(i, 3), p(i, 4),
                                p(i, 5), sp.e1, phase_e1, sp.strength,
                                sp.pref_b, sp.m_b);
      }
      ...
      FF_algorithm::bend_yoshida6<typename BP::part_t,
          ...
          p(i, 0), p(i, 1), p(i, 2), p(i, 3), p(i, 4), p(i, 5), sp.pref_b,
          sp.m_b, step_ref_cdt, step_kl, step_dphi, step_phase, step_term,
          sp.r0, sp.strength, sp.steps);

      if (sp.redge) {
        // bend edge (thin, but with face angle)
        FF_algorithm::bend_edge(p(i, 0), p(i, 1), p(i, 2), p(i, 3), p(i, 4),
                                p(i, 5), sp.e2, phase_e2, sp.strength,
                                sp.pref_b, sp.m_b);
...
template <class BP> struct PropSbendCFSimd {
  using gsv_t = typename BP::gsv_t;
  using parts_t = typename BP::parts_t;
  using const_masks_t = typename BP::const_masks_t;
  ...
  void operator()(const int idx) const {
    ...
    if (m) {
      ...
      if (sp.ledge) {
        ...
        // bend edge (thin, but with face angle)
        FF_algorithm::bend_edge<gsv_t>(p0, p1, p2, p3, p4, p5, sp.e1, phase_e1,
                                       sp.strength, sp.pref_b, sp.m_b);
      }

      // bend body
      FF_algorithm::bend_yoshida6<gsv_t, fa::thin_cf_kick_5<gsv_t>, 5>(
          p0, p1, p2, p3, p4, p5, sp.pref_b, sp.m_b, step_ref_cdt, step_kl,
          step_dphi, step_phase, step_term, sp.r0, sp.strength, sp.steps);

      if (sp.redge) {
        // bend edge (thin, but with face angle)
        FF_algorithm::bend_edge<gsv_t>(p0, p1, p2, p3, p4, p5, sp.e2, phase_e2,
                                       sp.strength, sp.pref_b, sp.m_b);
...
inline void prop_reference(Reference_particle &ref_l, SbendParams &sp) {
  ...
  Kokkos::complex<double> phase = std::exp(std::complex<double>(0.0, -dphi));
  ...
  if (sp.ledge) {
    // slot
    FF_algorithm::slot_unit(x_l, xp_l, y_l, yp_l, cdt_l, dpop_l, sp.ce1, sp.se1,
                            pref_l, m_l);
    ...
    FF_algorithm::edge_unit(y_l, xp_l, yp_l, sp.us_edge_k_x / sp.scale,
                            sp.us_edge_k_y / sp.scale, 0);
  }

  FF_algorithm::bend_complete(x_l, xp_l, y_l, yp_l, cdt_l, dpop_l, dphi,
                              sp.strength, pref_l, m_l, 0.0 /*ref cdt*/, phase,
                              term);
  ...
  if (sp.redge) {
    ...
    // edge kick strenth are scaled to bunch. so need to div by "scale" to scale
    // it to the lattice reference
    FF_algorithm::edge_unit(y_l, xp_l, yp_l, sp.ds_edge_k_x / sp.scale,
                            sp.ds_edge_k_y / sp.scale, 0);

    // slot
    FF_algorithm::slot_unit(x_l, xp_l, y_l, yp_l, cdt_l, dpop_l, sp.ce2, sp.se2,
                            pref_l, m_l);
  }
...
inline void prop_reference_cf(Reference_particle &ref_l, SbendParams &sp) {
  double pref_l = ref_l.get_momentum();
  double m_l = ref_l.get_mass();
  ...
  double step_kl[10];
  for (int i = 0; i < 10; ++i)
    step_kl[i] = sp.k_l[i] * step_length;
  ...
  if (sp.ledge) {
    ...
    // bend edge (thin)
    FF_algorithm::bend_edge(x_l, xp_l, y_l, yp_l, cdt_l, dpop_l, sp.e1,
                            phase_e1, sp.strength, pref_l, m_l);
  }

  FF_algorithm::bend_yoshida6<double, FF_algorithm::thin_cf_kick_5<double>, 5>(
      x_l, xp_l, y_l, yp_l, cdt_l, dpop_l, pref_l, m_l, 0.0 /* step ref_cdt */,
      step_kl, step_dphi, step_phase, step_term, sp.r0, sp.strength, sp.steps);

  if (sp.redge) {
    // bend edge (thin)
    FF_algorithm::bend_edge(x_l, xp_l, y_l, yp_l, cdt_l, dpop_l, sp.e2,
                            phase_e2, sp.strength, pref_l, m_l);
...
template <class BP> struct PropThinPoleSimd {
  using gsv_t = typename BP::gsv_t;
...
inline void apply(Lattice_element_slice const &slice, BunchT &bunch) {
  ...
  if ((sp.k_l[0] != 0.0) || (sp.k_l[1] != 0.0))
    cf = 1;
  if ((sp.k_l[2] != 0.0) || (sp.k_l[3] != 0.0))
    cf = 2;
  if ((sp.k_l[4] != 0.0) || (sp.k_l[5] != 0.0))
    cf = 3;
  if ((sp.k_l[6] != 0.0) || (sp.k_l[7] != 0.0))
    cf = 4;
  if ((sp.k_l[8] != 0.0) || (sp.k_l[9] != 0.0))
    cf = 5;
  ...
  // the particle dpop is with respect to this momentum which goes with the
  // bunch
  ...
  sp.us_edge_k =
      ((charge_l > 0) ? 1.0 : -1.0) * sp.strength * tan(sp.e1) / brho_b;
  sp.ds_edge_k =
      -((charge_l > 0) ? 1.0 : -1.0) * sp.strength * tan(-sp.e2) / brho_b;

  // edge kick (per) particle (full strength, angles are calculated at each
  // particle)
