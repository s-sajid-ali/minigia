...
namespace pp_impl {
template <class T>
using kf_t = void (*)(T const &, T &, T const &, T &, T const &,
                      double const *);
}

template <class BUNCH, class gsv_t, pp_impl::kf_t<gsv_t> kf_gsv,
          pp_impl::kf_t<double> kf_d, int COMP = 1>
struct FF_patterned_propagator {
  ...
  struct thin_kicker {
    ...
    thin_kicker(bp_t &bp, double const *str) : p(bp.parts), m(bp.masks) {
  ...
  struct simple_kicker {
    ...
    simple_kicker(bp_t &bp, double pref, double mass, double ref_cdt,
                  double len, double const *str)
        : p(bp.parts), m(bp.masks), pref(pref), mass(mass), ref_cdt(ref_cdt),
          len(len) {
      for (int i = 0; i < COMP * 2; ++i)
        k[i] = str[i];
    }
    ...
    void operator()(const int idx) const {
      ...
      if (mask) {
        ...
        FF_algorithm::drift_unit(p0, p1, p2, p3, p4, p5, len * 0.5, pref, mass,
                                 ref_cdt * 0.5);
  ...
  struct yoshida_kicker {
    ...
    yoshida_kicker(bp_t &bp, double pref, double mass, double step_ref_cdt,
                   double step_len, double const *step_str, int steps)
        : p(bp.parts), m(bp.masks), pref(pref), mass(mass),
          step_ref_cdt(step_ref_cdt), step_len(step_len), steps(steps) {
      for (int i = 0; i < COMP * 2; ++i)
        step_k[i] = step_str[i];
    }
    ...
    void operator()(const int idx) const {
      ...
      if (mask) {
        ...
        FF_algorithm::yoshida6<gsv_t, kf_gsv, COMP>(p0, p1, p2, p3, p4, p5,
  ...
  static void apply_thin_kick(BUNCH &bunch, ParticleGroup pg, double const *k) {
  ...
  static void apply_simple_kick(BUNCH &bunch, ParticleGroup pg, double pref,
                                double mass, double ref_cdt, double len,
                                double const *k) {
    if (!bunch.get_local_num(pg))
      return;
    ...
    simple_kicker sk(bunch.get_bunch_particles(pg), pref, mass, ref_cdt, len,
                     k);

    Kokkos::parallel_for(range, sk);
  }

  static void apply_yoshida_kick(BUNCH &bunch, ParticleGroup pg, double pref,
                                 double mass, double ref_cdt, double len,
                                 double const *str, int steps) {
    if (!bunch.get_local_num(pg))
      return;
    ...
    for (int i = 0; i < 2 * COMP; ++i)
      step_str[i] = str[i] * step_len;

    using exec = typename BUNCH::exec_space;
    auto range = Kokkos::RangePolicy<exec>(0, bunch.size_in_gsv(pg));

    yoshida_kicker yk(bunch.get_bunch_particles(pg), pref, mass, step_ref_cdt,
                      step_len, step_str, steps);

    Kokkos::parallel_for(range, yk);
  }

  static void get_reference_cdt_zero(Reference_particle &ref, double const *k) {
  ...
  static double get_reference_cdt_simple(Reference_particle &ref, double len,
                                         double const *k) {
    ...
    FF_algorithm::drift_unit(st[0], st[1], st[2], st[3], cdt, st[5], len * 0.5,
                             pref, mass, 0.0);
  ...
  // non zero length
  static double get_reference_cdt_yoshida(Reference_particle &ref, double len,
                                          double const *k, int steps,
    ...
    double step_str[2 * COMP];
    for (int i = 0; i < 2 * COMP; ++i)
      step_str[i] = k[i] * step_len;

    // propagate
    if (use_drift) {
      FF_algorithm::drift_unit(st[0], st[1], st[2], st[3], cdt, st[5], len,
                               pref, mass, 0.0);

    } else {
      // hardcoded to use 6th order
      FF_algorithm::yoshida<double, kf_d, 6, COMP>(st[0], st[1], st[2], st[3],
                                                   cdt, st[5], pref, mass, 0.0,
                                                   step_len, step_str, steps);
    }
