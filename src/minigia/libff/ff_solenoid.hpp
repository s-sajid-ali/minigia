...
namespace solenoid_impl {
  ...
  template <class BunchT> struct solenoid_unit_kicker {
    using gsv_t = typename BunchT::gsv_t;
    ...
    KOKKOS_INLINE_FUNCTION
      void operator()(const int idx) const {
        ...
        if (mask) {
          ...
          FF_algorithm::solenoid_unit(p0, p1, p2, p3, p4, p5, ksl, ks, length,
              ref_p, mass, ref_cdt);
  ...
  template <class BunchT>
    void apply_solenoid_unit(BunchT &bunch, ParticleGroup pg, double ksl, double ks,
        double length, double ref_p, double mass,
        double ref_cdt) {
      if (!bunch.get_local_num(pg))
        return;
      ...
      solenoid_unit_kicker<BunchT> sk{parts,  masks, ksl,  ks,
        length, ref_p, mass, ref_cdt};

      Kokkos::parallel_for(range, sk);
    }

  inline double get_reference_cdt_solenoid(double length,
      Reference_particle &reference_particle,
      bool in_edge, bool out_edge, double ks,
      double kse, double ksl) {
    ...
    // body
    FF_algorithm::solenoid_unit(x, xp, y, yp, cdt, dpop, ksl, ks, length, ref_p,
        mass, 0.0);
  ...
  template <class BunchT>
    void apply(Lattice_element_slice const &slice, BunchT &bunch) {
      ...
      double brho_l = ref_l.get_momentum() *
        (1.0 + ref_l.get_state()[Bunch::dpop]) /
      ...
      // reference cdt
      const double ref_cdt = get_reference_cdt_solenoid(length, ref_l, has_in_edge,
          has_out_edge, ks, kse, ksl);

      using gsv_t = typename BunchT::gsv_t;

      // in-edge
      if (has_in_edge) {
        apply_edge_kick<BunchT, FF_algorithm::solenoid_in_edge_kick<gsv_t>>(
      ...
      // body
      apply_solenoid_unit(bunch, ParticleGroup::regular, ksl, ks, length, ref_p,
          mass, ref_cdt);

      apply_solenoid_unit(bunch, ParticleGroup::spectator, ksl, ks, length, ref_p,
          mass, ref_cdt);

      // out-edge
      if (has_out_edge) {
        apply_edge_kick<BunchT, FF_algorithm::solenoid_out_edge_kick<gsv_t>>(
            bunch, ParticleGroup::regular, kse);
