...
struct propagator_fixture {
  ...
  propagator_fixture(std::string const &file)
    ...
    sim = std::make_unique<Bunch_simulator>(
        Bunch_simulator::create_single_bunch_simulator(ref_b, 16, 1e10,
                                                       Commxx()));
  }
...
void propagate_libff(std::string const &seq,
  ...
  for (int p = 0; p < 16; ++p) {
    ...
    CHECK(parts(p, 5) == Approx(de_to_dp(madx(p, 5))).margin(tolerance));
#endif
  }
}

TEST_CASE("drift", "[libFF][Elements]") { propagate_libff("drift"); }

TEST_CASE("quad", "[libFF][Elements]") { propagate_libff("quad"); }

TEST_CASE("sextupole", "[libFF][Elements]") { propagate_libff("sext"); }

TEST_CASE("octupole", "[libFF][Elements]") { propagate_libff("oct"); }
...
TEST_CASE("octupole_long", "[libFF][Elements]") { propagate_libff("oct_long"); }

TEST_CASE("kicker", "[libFF][Elements]") { propagate_libff("kicker"); }

TEST_CASE("hkicker", "[libFF][Elements]") { propagate_libff("hkicker"); }

TEST_CASE("vkicker", "[libFF][Elements]") { propagate_libff("vkicker"); }
...
TEST_CASE("rfc", "[libFF][Elements]") { propagate_libff("rfc"); }

TEST_CASE("solenoid", "[libFF][Elements]") { propagate_libff("solenoid"); }
