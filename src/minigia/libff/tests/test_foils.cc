...
#include <minigia/bunch/populate.hpp>
...
struct propagator_fixture {
  ...
  propagator_fixture(std::string const &seq)
      : screen(0, LoggerV::INFO_TURN),
        ...
        propagator(lattice, Independent_stepper_elements(1)), sim() {
    ...
    sim = std::make_unique<Bunch_simulator>(
        Bunch_simulator::create_single_bunch_simulator(ref, 1, 1e09, Commxx()));
...
TEST_CASE("foil") {
  ...
  std::cout << "pref = " << ref.get_momentum() << ", beta = " << ref.get_beta()
            << "\n";
