...
inline bool close_to_zero(double v) { return fabs(v) < 1e-13; }
...
namespace FF_algorithm {
  ...
  template <typename T> KOKKOS_INLINE_FUNCTION T invsqrt(T const &x) {
    return 1.0 / sqrt(x);
  }
  ...
  template <typename T>
    KOKKOS_INLINE_FUNCTION void drift_unit(T &x, T const &xp, T &y, T const &yp,
        T &cdt, T const &dpop, double length,
        double reference_momentum, double m,
  ...
  template <typename T>
    KOKKOS_INLINE_FUNCTION void slot_unit(T &x, T &xp, T &y, T &yp, T &cdt, T &dpop,
        double ct, double st, double pref,
        double m) {
      const T uni(1.0);
  ...
  template <typename T>
    KOKKOS_INLINE_FUNCTION void edge_unit(T const &y, T &yp, T k) {
      yp = yp - k * y;
    }

  template <typename T>
    KOKKOS_INLINE_FUNCTION void edge_unit(T const &y, T &xp, T &yp, double kx,
        double ky, char) {
      const T vkx(kx);
      const T vky(ky);
  ...
  template <typename T>
    KOKKOS_INLINE_FUNCTION void edge_unit(T const &y, T &xp, T &yp, T const &dpop,
        T k) {
      const T uni(1.0);
  ...
  template <typename T>
    KOKKOS_INLINE_FUNCTION void dipole_unit(T &x, T &xp, T &y, T &yp, T &cdt,
        T const &dpop, double l, double k0) {
  ...
  template <typename T>
    ...
    bend_complete(T &x, T &xp, T &y, T &yp, T &cdt, T const &dpop, double dphi,
        double strength, double p_ref, double m, double cdt_ref,
  ...
  template <typename T>
    KOKKOS_INLINE_FUNCTION void
    bend_unit(T &x, T &xp, T &y, T &yp, T &cdt, T const &dpop, double theta,
        double strength, double p_ref, double m, double cdt_ref,
        Kokkos::complex<double> phase, Kokkos::complex<double> term) {
      typedef Kokkos::complex<T> CT;
  ...
  template <typename T>
    KOKKOS_INLINE_FUNCTION void
    bend_edge(T &x, T &xp, T &y, T &yp, T &cdt, T const &dpop, T e,
        Kokkos::complex<double> phase, T strength, T p_ref, T m) {
      typedef Kokkos::complex<T> CT;
  ...
  template <typename T>
    KOKKOS_INLINE_FUNCTION void
    thin_cf_quadrupole_unit(T const &x, T &xp, T const &y, T &yp, double r0,
        double const *kL) {
      T vk0(kL[0]);
      T vk1(kL[1]);
...
#define MODEL 10
  ...
  // Zolkin, Sector magnets or transverse electromagnetic fields in cylindrical
  // coordinates, Phys.Rev.Accel.Beams 20 (2017) no.4, 043501 Table XIII
  template <typename T>
    KOKKOS_INLINE_FUNCTION T thin_cf_quadrupole_bx(T const &x, T const &y,
        T const &r0) {
      return r0 * y / (r0 + x);
    }

  template <typename T>
    KOKKOS_INLINE_FUNCTION T thin_cf_quadrupole_by(T const &x, T const &y,
        T const &r0, T const &alf) {
  ...
  // coordinates, Phys.Rev.Accel.Beams 20 (2017) no.4, 043501 Table XIII
  template <typename T>
    KOKKOS_INLINE_FUNCTION T thin_cf_sextupole_bx(T const &x, T const &y,
        T const &r0, T const &alf) {
      return x * (T(2.0) + alf) * y / (T(1.0) + alf);
    }

  template <typename T>
    KOKKOS_INLINE_FUNCTION T thin_cf_sextupole_by(T const &x, T const &y,
        T const &r0, T const &alf) {
  ...
  template <typename T>
    KOKKOS_INLINE_FUNCTION void thin_cf_kick_1(T const &x, T &xp, T const &y, T &yp,
        double r0, double const *kL) {
      const T uni(1.0);
  ...
  template <typename T>
    KOKKOS_INLINE_FUNCTION void thin_cf_kick_2(T const &x, T &xp, T const &y, T &yp,
        double r0, double const *kL) {
      const T uni(1.0);
  ...
  // cylindrical coordinates, Phys.Rev.Accel.Beams 20 (2017) no.4, 043501 Table
  // XII
  template <typename T>
    KOKKOS_INLINE_FUNCTION T F_rn_2(T const &rho, T const &y) {
      return T(-1.0) * (y / rho);
    }

  template <typename T>
    KOKKOS_INLINE_FUNCTION T F_rn_3(T const &rho, T const &y) {
      return T(-0.5) * (y / rho) * (rho * rho - T(1.0));
    }

  template <typename T>
    KOKKOS_INLINE_FUNCTION T F_rn_4(T const &rho, T const &y) {
  ...
  template <typename T>
    KOKKOS_INLINE_FUNCTION T F_rn_5(T const &rho, T const &y) {
      return T(-1.0 / 12.0) * (y / rho) *
        ((rho * rho - T(1.0)) * (T(3.0 / 4.0) * (rho * rho + T(1.0)) - y * y) -
         T(3.0) * rho * rho * log(rho));
    }

  template <typename T>
    KOKKOS_INLINE_FUNCTION T F_yn_2(T const &rho, T const &y) {
      return log(rho);
    }

  template <typename T>
    KOKKOS_INLINE_FUNCTION T F_yn_3(T const &rho, T const &y) {
      return T(0.5) * (T(0.5) * (rho * rho - T(1.0)) - y * y - log(rho));
    }

  template <typename T>
    KOKKOS_INLINE_FUNCTION T F_yn_4(T const &rho, T const &y) {
      return T(-0.25) * (rho * rho - T(1.0)) +
        T(0.5) * (T(0.5) * (rho * rho + T(1.0)) - y * y) * log(rho);
    }

  template <typename T>
    KOKKOS_INLINE_FUNCTION T F_yn_5(T const &rho, T const &y) {
      return T(0.125) *
        (T(0.125) * (rho * rho * rho * rho + T(4.0) * rho * rho - T(5.0)) -
         (rho * rho - T(1.0)) * y * y + y * y * y * y -
         (T(0.5) + rho * rho - T(2.0) * y * y) * log(rho));
    }

  template <typename T>
    KOKKOS_INLINE_FUNCTION T F_rs_2(T const &rho, T const &y) {
      return T(0.5) / rho * (rho * rho - T(1.0));
    }

  template <typename T>
    KOKKOS_INLINE_FUNCTION T F_rs_3(T const &rho, T const &y) {
      return T(0.5) / rho *
        (T(-0.5) * (rho * rho - T(1.0)) - y * y + rho * rho * log(rho));
    }

  template <typename T>
    KOKKOS_INLINE_FUNCTION T F_rs_4(T const &rho, T const &y) {
      return T(0.25) / rho *
  ...
  template <typename T>
    KOKKOS_INLINE_FUNCTION T F_rs_5(T const &rho, T const &y) {
      return T(0.125) / rho *
  ...
  template <typename T>
    KOKKOS_INLINE_FUNCTION T F_ys_2(T const &rho, T const &y) {
      return y;
    }

  template <typename T>
    KOKKOS_INLINE_FUNCTION T F_ys_3(T const &rho, T const &y) {
      return y * log(rho);
    }

  template <typename T>
    KOKKOS_INLINE_FUNCTION T F_ys_4(T const &rho, T const &y) {
      return T(0.5) * y *
        (T(0.5) * (rho * rho - T(1.0)) - y * y / T(3.0) - log(rho));
    }

  template <typename T>
    KOKKOS_INLINE_FUNCTION T F_ys_5(T const &rho, T const &y) {
  ...
  template <typename T>
    KOKKOS_INLINE_FUNCTION void thin_cf_kick_5(T const &x, T &xp, T const &y, T &yp,
        double r0, double const *kL) {
      const T uni(1.0);
  ...
  template <typename T>
    KOKKOS_INLINE_FUNCTION void
    quadrupole_chef(T &x, T &xp, T &y, T &yp, T &cdt, T const &dpop, double pref,
        double m, double refcdt, double len, double *str, int kicks) {
      ...
      if (kicks == 1) {
        ...
        for (int i = 0; i < u; ++i) {
          drift_unit(x, xp, y, yp, cdt, dpop, frontLength, pref, m,
              substep_ref_cdt);
          thin_quadrupole_unit(x, xp, y, yp, kl); // quaterStrength

          for (int i = 0; i < 3; ++i) {
            drift_unit(x, xp, y, yp, cdt, dpop, sepLength, pref, m,
                substep_ref_cdt);
            thin_quadrupole_unit(x, xp, y, yp, kl); // quaterStrength
          }
        ...
        drift_unit(x, xp, y, yp, cdt, dpop, len / (2.0 * kicks), pref, m,
            substep_ref_cdt);
        thin_quadrupole_unit(x, xp, y, yp, kl); // str*len/kicks

        for (int i = 0; i < kicks - 1; ++i) {
          drift_unit(x, xp, y, yp, cdt, dpop, len / kicks, pref, m,
              substep_ref_cdt);
          thin_quadrupole_unit(x, xp, y, yp, kl); // str*len/kicks
        }
  ...
  // thin kick dipole
  template <typename T>
    KOKKOS_INLINE_FUNCTION void thin_dipole_unit(T const &x, T &xp, T const &y,
        T &yp, double const *kL) {
      xp = xp - T(kL[0]);
      yp = yp + T(kL[1]);
    }

  template <typename T>
    KOKKOS_INLINE_FUNCTION void thin_quadrupole_unit(T const &x, T &xp, T const &y,
        T &yp, double const *kL) {
      T vk0(kL[0]);
      T vk1(kL[1]);
  ...
  template <typename T>
    KOKKOS_INLINE_FUNCTION void thin_sextupole_unit(T const &x, T &xp, T const &y,
        T &yp, double const *kL) {
      T vk0(kL[0]);
      T vk1(kL[1]);
  ...
  template <typename T>
    KOKKOS_INLINE_FUNCTION void thin_octupole_unit(T const &x, T &xp, T const &y,
        T &yp, double const *kL) {
      T vk0(kL[0]);
      T vk1(kL[1]);
  ...
  template <typename T>
    KOKKOS_INLINE_FUNCTION void thin_rbend_unit(T const &x, T &xp, T const &y,
        T &yp, double const *kL) {
  ...
  template <typename T>
    KOKKOS_INLINE_FUNCTION void rbend_thin_cf_kick(T const &x, T &xp, T const &y,
        T &yp, double r0,
        double const *kL) {
  ...
  template <typename T>
    KOKKOS_INLINE_FUNCTION void thin_kicker_unit(T &p, double kL) {
      p = p + T(kL);
    }

  template <typename T>
    KOKKOS_INLINE_FUNCTION void thin_kicker_unit(T &xp, T &yp, double const *kL) {
      xp = xp + T(kL[0]);
      yp = yp + T(kL[1]);
    }

  template <typename T>
    KOKKOS_INLINE_FUNCTION void constfoc_unit(T &x, T &xp, double cs, double sn,
  ...
  KOKKOS_INLINE_FUNCTION
    double thin_rfcavity_pnew(double pref, double m, double volt, double phi_s) {
  ...
  template <typename T>
    KOKKOS_INLINE_FUNCTION void
    thin_rfcavity_unit(T &px, T &py, T const &cdt, T &dpop, double w_rf,
        double volt, double phi_s, double m, double old_ref_p,
        double new_ref_p, double const *mhp, int nh) {
      ...
      for (int i = 0; i < nh; ++i) {
        ...
        strength_factor =
          strength_factor +
          T(mhp_relative_strength) *
          sin(T(mhp_harmonic_multiple) * (T(phi_s) + phase_slip_argument) +
              T(mhp_phase_shift));
      }
  ...
  template <typename T>
    KOKKOS_INLINE_FUNCTION void thin_magnet_unit(T const &x, T &xp, T const &y,
        T &yp, double const *kL, int n) {
      for (int k = 0; k < n; k += 4) {
  ...
  template <typename T>
    ...
    template <class T>
    KOKKOS_INLINE_FUNCTION void nllens_unit(T const &x, T &xp, T const &y, T &yp,
        T const &, double const *k) {
      const T icnll = T(k[0]);
      const T kick = T(k[1]);
      ...
      T theta = (co_r > 0) ? atan(co_i / co_r) + T(0.0)
      ...
      // df2 = carcsin/cr2
      T df2_r =
        (cr2_r * carcsin_r + cr2_i * carcsin_i) / (cr2_r * cr2_r + cr2_i * cr2_i);
      T df2_i =
        (cr2_r * carcsin_i - cr2_i * carcsin_r) / (cr2_r * cr2_r + cr2_i * cr2_i);
  ...
  template <typename T>
    KOKKOS_INLINE_FUNCTION void dipedge_unit(T &x, T &xp, T &y, T &yp,
        double re_2_1, double re_4_3,
        double const *te) {
      // linear terms
      xp = xp + T(re_2_1) * x;
      yp = yp + T(re_4_3) * y;
      ...
      xp = xp + T(te[2]) * x * x + T(te[4]) * y * y + T(te[3]) * x * xp * T(2) +
        T(te[5]) * y * yp * T(2);
      y = y + T(te[6]) * x * y * T(2);
  ...
  template <class T>
    KOKKOS_INLINE_FUNCTION void solenoid_unit(T &x, T &xp, T &y, T &yp, T &cdt,
        T const &dpop, double ks, double ksl,
        double length, double ref_p,
  ...
  template <class T>
    KOKKOS_INLINE_FUNCTION void solenoid_in_edge_kick(T const &x, T &xp, T const &y,
        T &yp, double kse) {
      xp = xp + T(kse) * y;
      yp = yp - T(kse) * x;
    }
  ...
  template <class T>
    KOKKOS_INLINE_FUNCTION void elens_kick_gaussian(T const &x, T &xp, T const &y,
        T &yp, T const &dpop,
        double const *k) {
      ...
      if (r < small_radius) {
        kick =
          ...
          (r / (t2 * radius * radius) -
           ...
           (t1 / t6) * ((r * r * r * r * r) / (t8 * radius * radius * radius *
  ...
  template <class T>
    KOKKOS_INLINE_FUNCTION void elens_kick_uniform(T const &x, T &xp, T const &y,
        T &yp, T const &dpop,
        double const *k) {
      // const double small_radius = 1.0e-10;
      T r = sqrt(x * x + y * y);
  ...
  template <class T>
    using yoshida_kf_t = void (*)(T const &, T &, T const &, T &, T const &,
        double const *);

  template <class T>
    using bend_yoshida_kf_t = void (*)(T const &, T &, T const &, T &, double,
        double const *);

  template <typename T, yoshida_kf_t<T> kf, int n, int components>
    struct yoshida_element {
      KOKKOS_INLINE_FUNCTION
        static void integral(T &x, T &xp, T &y, T &yp, T &cdt, T const &dpop,
          ...
          yoshida_element<T, kf, n - 1, components>::integral(
              x, xp, y, yp, cdt, dpop, pref, m, substep_ref_cdt, step_length,
              step_strength, steps, c * x1(n));

          yoshida_element<T, kf, n - 1, components>::integral(
              x, xp, y, yp, cdt, dpop, pref, m, substep_ref_cdt, step_length,
              step_strength, steps, c * x0(n));
      ...
      KOKKOS_INLINE_FUNCTION
        static double x0(int nn) { return 1.0 - 2.0 * x1(nn); }
    };
    ...
    struct yoshida_element<T, kf, 0, components> {
      KOKKOS_INLINE_FUNCTION
        static void integral(T &x, T &xp, T &y, T &yp, T &cdt, T const &dpop,
          ...
          drift_unit(x, xp, y, yp, cdt, dpop, 0.5 * c * step_length, pref, m,
              substep_ref_cdt);
    ...
    KOKKOS_INLINE_FUNCTION struct yoshida_element<T, kf, 0, components> {
      KOKKOS_INLINE_FUNCTION
        static void integral(T &x, T &xp, T &y, T &yp, T &cdt, T const &dpop,
          ...
          drift_unit(x, xp, y, yp, cdt, dpop, c * step_length, pref, m,
              substep_ref_cdt);
  ...
  template <typename T, yoshida_kf_t<T> kf, int order, int components>
    KOKKOS_INLINE_FUNCTION void yoshida(T &x, T &xp, T &y, T &yp, T &cdt,
        T const &dpop, double pref, double m,
        double step_ref_cdt, double step_length,
        double const *step_strength, int steps) {
      const int n = (order - 2) / 2;

      for (int i = 0; i < steps; ++i) {
        yoshida_element<T, kf, n, components>::integral(
            x, xp, y, yp, cdt, dpop, pref, m, step_ref_cdt, step_length,
            step_strength, steps, 1.0);
      }
    }
  ...
  template <typename T, yoshida_kf_t<T> kf, int components>
    KOKKOS_INLINE_FUNCTION void
    yoshida2(T &x, T &xp, T &y, T &yp, T &cdt, T const &dpop,
        double reference_momentum, double m, double substep_reference_cdt,
        double step_length, double const *step_strength, int steps) {
      for (int i = 0; i < steps; ++i) {
  ...
  template <typename T, yoshida_kf_t<T> kf, int components>
    KOKKOS_INLINE_FUNCTION void
    yoshida4(T &x, T &xp, T &y, T &yp, T &cdt, T const &dpop,
        double reference_momentum, double m, double step_reference_cdt,
        double step_length, double const *step_strength, int steps) {
      ...
      for (int i = 0; i < steps; ++i) {
        drift_unit(x, xp, y, yp, cdt, dpop, c1 * step_length, reference_momentum, m,
            substep_reference_cdt);

        // kf( x, xp, y, yp, d1 * step_strength );
        kf(x, xp, y, yp, cdt, k1);

        drift_unit(x, xp, y, yp, cdt, dpop, c2 * step_length, reference_momentum, m,
            substep_reference_cdt);

        // kf( x, xp, y, yp, d2 * step_strength );
        kf(x, xp, y, yp, cdt, k2);

        drift_unit(x, xp, y, yp, cdt, dpop, c3 * step_length, reference_momentum, m,
            substep_reference_cdt);

        // kf( x, xp, y, yp, d3 * step_strength );
        kf(x, xp, y, yp, cdt, k3);

        drift_unit(x, xp, y, yp, cdt, dpop, c4 * step_length, reference_momentum, m,
            substep_reference_cdt);
      }
    }
  ...
  template <typename T, yoshida_kf_t<T> kf, int components>
    KOKKOS_INLINE_FUNCTION void
    yoshida6(T &x, T &xp, T &y, T &yp, T &cdt, T const &dpop,
        double reference_momentum, double m, double step_reference_cdt,
        double step_length, double const *step_strength, int steps) {
      // see yoshida4.py for formulas
  ...
  template <typename T,
           void(kf)(T const &x, T &xp, T const &y, T &yp, double r0,
               double const *kL),
           int components>
             KOKKOS_INLINE_FUNCTION void
             bend_yoshida4(T &x, T &xp, T &y, T &yp, T &cdt, T const &dpop,
                 double reference_momentum, double m, double step_reference_cdt,
                 double step_angle, double const *step_strength, double r0,
                 double bend_strength, int steps) {
#if 0
               // see yoshida4.py for formulas
               const double c1 = 0.675603595979828817023843904487;
               const double c4 = c1;
  ...
  KOKKOS_INLINE_FUNCTION
    double sbend_dphi(double c, double delta) { return -c * delta; }

  KOKKOS_INLINE_FUNCTION
    double rbend_dphi(double c, double delta) { return 0.0; }

#if 1
  // hardwired 6th order yoshida
  template <typename T,
           void(f_kf)(T const &x, T &xp, T const &y, T &yp, double r0,
               double const *kL), // kick
           int components>
             KOKKOS_INLINE_FUNCTION void
             bend_yoshida6(T &x, T &xp, T &y, T &yp, T &cdt, T const &dpop,
                 double reference_momentum, double m, double step_reference_cdt,
                 double const *step_strength, double const *dphi,
                 Kokkos::complex<double> const *step_phase,
                 Kokkos::complex<double> const *step_term, double r0,
                 double bend_strength, int steps) {
               // see yoshida4.py for formulas
               ...
               for (int i = 0; i < steps; ++i) {
                 ...
                 bend_unit<T>(x, xp, y, yp, cdt, dpop, dphi[0], bend_strength,
                     reference_momentum, m, substep_reference_cdt, step_phase[0],
                     step_term[0]);
                 ...
                 bend_unit<T>(x, xp, y, yp, cdt, dpop, dphi[1], bend_strength,
                     reference_momentum, m, substep_reference_cdt, step_phase[1],
                     step_term[1]);
                 ...
                 bend_unit<T>(x, xp, y, yp, cdt, dpop, dphi[2], bend_strength,
                     reference_momentum, m, substep_reference_cdt, step_phase[2],
                     step_term[2]);
                 ...
                 bend_unit<T>(x, xp, y, yp, cdt, dpop, dphi[3], bend_strength,
                     reference_momentum, m, substep_reference_cdt, step_phase[3],
                     step_term[3]);
