...
namespace rfcavity_impl {
  ...
  template <class BunchT> struct PropThinRFCavity {
    ...
    KOKKOS_INLINE_FUNCTION
      void operator()(const int idx) const {
        ...
        if (mask) {
          ...
          FF_algorithm::thin_rfcavity_unit(p1, p3, p4, p5, rp.w_rf, rp.str,
              rp.phi_s, rp.m_b, rp.pref_b,
              rp.new_pref_b, rp.mhp, rp.nh);
  ...
  template <class BunchT> struct PropRFCavity {
    using gsv_t = typename BunchT::gsv_t;
    ...
    KOKKOS_INLINE_FUNCTION
      void operator()(const int idx) const {
        ...
        if (mask) {
          ...
          FF_algorithm::drift_unit(p0, p1, p2, p3, p4, p5, 0.5 * rp.length,
              rp.pref_b, rp.m_b, rp.ref_cdt_1);
...
namespace FF_rfcavity {
  template <class BunchT>
    void apply(Lattice_element_slice const &slice, BunchT &bunch) {
      ...
      rp.new_pref_b =
        FF_algorithm::thin_rfcavity_pnew(rp.pref_b, rp.m_b, rp.str, rp.phi_s);
      ...
      // first half drift
      FF_algorithm::drift_unit(ref_l_x, ref_l_xp, ref_l_y, ref_l_yp, ref_l_cdt,
          ref_l_dpop, 0.5 * rp.length, ref_l_p, ref_l_m, 0.0);

      double total_ref_cdt = ref_l_cdt;
      rp.ref_cdt_1 = ref_l_cdt;
      ref_l_cdt = 0.0;

      // do not give it the new_ref_l_p because the xp and yp dont get scaled, the
      // momentum of the lattice reference particle remains unchanged, only the dpop
      // of the state has been changed
      FF_algorithm::thin_rfcavity_unit(ref_l_xp, ref_l_yp, ref_l_cdt, ref_l_dpop,
          rp.w_rf, rp.str, rp.phi_s, ref_l_m, ref_l_p,
          ref_l_p, rp.mhp, rp.nh);
      ...
      // save the state
      ref_l.set_state(ref_l_x, ref_l_xp, ref_l_y, ref_l_yp, total_ref_cdt,
          ref_l_dpop);
