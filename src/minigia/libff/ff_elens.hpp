...
namespace FF_elens {
  template <class BunchT>
    void apply(Lattice_element_slice const &slice, BunchT &bunch) {
      ...
      const double k[6] = {beta_b, gamma_b, beta_e, current_over_e, length, radius};

      using gsv_t = typename std::conditional<
        std::is_floating_point<typename BunchT::part_t>::value, GSVec<double>,
        typename BunchT::gsv_t>::type;

      if (gaussian) {
        using pp =
          FF_patterned_propagator<BunchT, gsv_t,
        ...
        if (close_to_zero(length)) {
          ...
          double ref_cdt = pp::get_reference_cdt_simple(ref_l, length, k);

          pp::apply_simple_kick(bunch, ParticleGroup::regular, pref_b, m_b, ref_cdt,
              length, k);

          pp::apply_simple_kick(bunch, ParticleGroup::spectator, pref_b, m_b,
              ref_cdt, length, k);
        }
      } else {
        using pp =
