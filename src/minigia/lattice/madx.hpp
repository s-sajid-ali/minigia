...
enum MadX_sequence_refer { SEQ_REF_ENTRY, SEQ_REF_CENTRE, SEQ_REF_EXIT };
...
std::string to_string(MadX_value const &val);
} // namespace synergia
...
public:
  MadX_command(MadX *parent = NULL)
      : mx(parent), name_(), label_(), type_(NULL_COMMAND), attributes_() {}
...
public:
  MadX_line(MadX const &parent) : parent(parent), elements_() {}
...
private:
  struct seq_element {
    seq_element(std::string const &label_, mx_expr const &at_,
...
public:
  ...
  void add_element(string_t const &label, mx_expr const &at,
                   string_t const &from);
  void finalize();
  void reset();
