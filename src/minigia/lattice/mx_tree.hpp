...
enum mx_line_member_type { MX_LINE_MEMBER_NAME, MX_LINE_MEMBER_SEQ };
...
namespace detail {
template <typename T> void print(T const &t) { t.print(); }
} // namespace detail
} // namespace synergia
...
public:
  mx_tree() : statements() {}
  mx_tree(mx_statements_t const &sts) : statements(sts) {}
...
public:
  mx_attr() : type_(MX_ATTR_NULL), name_(), value_() {}
...
public:
  mx_command()
      : type_(MX_CMD_VARIABLE), labeled_(false), label_(), keyed_(false),
        keyword_(), attrs_() {}
...
public:
  mx_if_block() : logic_expr(), block(), valid_(false) {}
