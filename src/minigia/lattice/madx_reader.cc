...
void insert_line(Lattice &lattice, MadX const &mx, std::string const &line_name,
                 bool dynamic = false) {
      ...
      for (auto const &name : attr_names) {
        ...
        case NUMBER:
          if (dynamic)
            ...
            element.set_double_attribute(name,
                                         cmd.attribute_as_number(name, 0.0));
          break;

        case ARRAY:
          element.set_vector_attribute(name,
                                       cmd.attribute_as_number_seq(name, 0.0));
          break;

        default:
          throw std::runtime_error("unable to process attribute " + name +
                                   " of element " + name);
        }
      }
...
double insert_sequence(Lattice &lattice, MadX const &mx,
                       std::string const &line_name, bool dynamic = false) {
  ...
  if (ref == SEQ_REF_ENTRY)
    r = 1.0;
  else if (ref == SEQ_REF_CENTRE)
    r = 0.5;
  else
    r = 0.0;
  ...
  for (int i = 0; i < sequence.element_count(); ++i) {
    ...
    if (sequence.element_type(i) == ENTRY_SEQUENCE) {
      ...
      current_pos =
          at + from + l; // sub-sequence always refer to the entry point
    ...
    for (std::vector<string_t>::iterator it = attribute_names.begin();
      ...
      case ARRAY:
        element.set_vector_attribute(*it,
                                     cmd.attribute_as_number_seq(*it, 0.0));
        break;
      default:
    ...
    double drift_length =
        at + from - current_pos - element.get_length() * (1.0 - r);
...
void extract_reference_particle(Lattice &lattice, MadX const &mx) {
  try {
...
void MadX_reader::parse(std::string const &string) {
  synergia::parse_madx(string, madx);
}

void MadX_reader::parse_file(std::string const &filename) {
  synergia::parse_madx_file(filename, madx);
}

std::vector<std::string> MadX_reader::get_line_names() const {
  return madx.line_labels();
}

std::vector<std::string> MadX_reader::get_sequence_names() const {
  return madx.sequence_labels();
}

std::vector<std::string> MadX_reader::get_all_names() const {
...
double MadX_reader::get_double_variable(std::string const &name) const {
  return madx.variable_as_number(name);
}

std::string MadX_reader::get_string_variable(std::string const &name) const {
  return madx.variable_as_string(name);
}

// construct the regular lattice
Lattice MadX_reader::get_lattice(std::string const &line_name) {
...
Lattice MadX_reader::get_lattice(std::string const &line_name,
  ...
  if (std::find(lines.begin(), lines.end(), line_name) != lines.end()) {
    ...
    if (std::find(seq.begin(), seq.end(), line_name) != seq.end()) {
      ...
      throw std::runtime_error("MadX_reader::get_lattice: "
                               "cannot find line or sequence with label " +
                               line_name);
    }
  }
...
Lattice MadX_reader::get_dynamic_lattice(std::string const &line_name) {
