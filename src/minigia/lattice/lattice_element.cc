...
Lattice_element::Lattice_element()
    : name(""), format(element_format::madx), stype("generic"),
      type(find_type(stype)), ancestors(), string_attributes(),
      lazy_double_attributes(), lazy_vector_attributes(),
      length_attribute_name("l"), bend_angle_attribute_name("angle"),
      revision(0), lattice_ptr(nullptr), markers{} {}

Lattice_element::Lattice_element(std::string const &type,
                                 std::string const &name, element_format format)
    : name(name), format(format), stype(type), type(find_type(stype)),
      ancestors(), string_attributes(), lazy_double_attributes(),
      lazy_vector_attributes(), length_attribute_name("l"),
      bend_angle_attribute_name("angle"), revision(0),
      lattice_ptr(nullptr), markers{} {}
...
Lsexpr Lattice_element::as_lsexpr() const {
  Lsexpr retval;
#if 0
...
std::vector<std::string> Lattice_element::get_all_type_names() {
...
std::string const &Lattice_element::get_type_name() const { return stype; }

element_type Lattice_element::get_type() const { return type; }

std::string const &Lattice_element::get_name() const { return name; }

element_format Lattice_element::get_format() const { return format; }

void Lattice_element::add_ancestor(std::string const &ancestor) {
  ancestors.push_back(ancestor);
}

std::list<std::string> const &Lattice_element::get_ancestors() const {
  return ancestors;
}

void Lattice_element::duplicate_attribute(std::string const &name,
...
void Lattice_element::delete_attribute(std::string const &name) {
  lazy_double_attributes.erase(name);
  lazy_vector_attributes.erase(name);
  string_attributes.erase(name);
}

void Lattice_element::rename_attribute(std::string const &name,
                                       std::string const &new_name) {
  duplicate_attribute(name, new_name);
  delete_attribute(name);
}

void Lattice_element::copy_attributes_from(Lattice_element const &o) {
...
void Lattice_element::set_double_attribute(std::string const &name,
                                           double value,
...
void Lattice_element::set_default_double_attribute(std::string const &name,
                                                   double value,
                                                   bool increment_revision) {
...
bool Lattice_element::has_double_attribute(std::string const &name) const {
  return lazy_double_attributes.count(name) > 0;
}

double Lattice_element::get_double_attribute(std::string const &name) const {
...
double Lattice_element::get_double_attribute(std::string const &name,
                                             double val) const {
  auto lr = lazy_double_attributes.find(name);
...
std::vector<std::string> Lattice_element::get_double_attribute_names() const {
...
std::map<std::string, double> Lattice_element::get_double_attributes() const {
...
void Lattice_element::set_string_attribute(std::string const &name,
                                           std::string const &value,
                                           bool increment_revision) {
...
void Lattice_element::set_default_string_attribute(std::string const &name,
                                                   std::string const &value,
                                                   bool increment_revision) {
...
bool Lattice_element::has_string_attribute(std::string const &name) const {
...
void Lattice_element::set_vector_attribute(std::string const &name,
...
bool Lattice_element::has_vector_attribute(std::string const &name) const {
...
void Lattice_element::set_length_attribute_name(
    std::string const &attribute_name) {
  length_attribute_name = attribute_name;
}

void Lattice_element::set_bend_angle_attribute_name(
    std::string const &attribute_name) {
  bend_angle_attribute_name = attribute_name;
}

double Lattice_element::get_length() const {
  return get_double_attribute(length_attribute_name, 0.0);
}

double Lattice_element::get_bend_angle() const {
  return get_double_attribute(bend_angle_attribute_name, 0.0);
}

long int Lattice_element::get_revision() const { return revision; }

bool Lattice_element::has_lattice() const { return (lattice_ptr != 0); }

void Lattice_element::set_lattice(Lattice &lattice) { lattice_ptr = &lattice; }

Lattice const &Lattice_element::get_lattice() const {
  if (!has_lattice()) {
...
namespace {
void validate_tunes_corrector(Lattice_element const &e) {
  // TODO: quads, CFbends, multipoles
  // for now, quads without tilt or skew
  // throw if not a valid corrector
}

void validate_chrom_corrector(Lattice_element const &e) {
...
void Lattice_element::set_marker(marker_type t) {
  switch (t) {
...
std::string Lattice_element::as_string() const {
...
void Lattice_element::print() const { std::cout << as_string() << std::endl; }

std::string Lattice_element::as_madx() const {
  std::stringstream ss;
  ss << name << ": " << stype;
