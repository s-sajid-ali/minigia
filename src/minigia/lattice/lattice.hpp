...
public:
  struct update_flags_t {
    ...
    template <class AR> void serialize(AR &ar) { ar(ref, structure, element); }
  };
...
public:
  ...
  bool is_dynamic_lattice() const { return true; }
  ...
  /// affect dynamics
  void set_all_double_attribute(std::string const &name, double value,
  ...
  void save_deposited_charge(std::string const &fname, int bunch_idx = 0,
