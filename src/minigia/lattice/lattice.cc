...
Lattice::update_flags_t Lattice::update() {
...
void Lattice::append(Lattice_element const &element) {
...
std::list<Lattice_element> const &Lattice::get_elements() const {
  return elements;
}

std::list<Lattice_element> &Lattice::get_elements() { return elements; }

void Lattice::reset_all_markers() {
  for (auto &e : elements)
    e.reset_markers();
}
...
double Lattice::get_total_angle() const {
  double angle = 0.0;
  for (auto const &e : elements)
...
std::string Lattice::as_string() const {
...
void Lattice::print(Logger &logger) const {
  logger(LoggerV::DEBUG) << as_string() << std::endl;
}

void Lattice::export_madx_file(std::string const &filename) const {
...
Lattice_tree const &Lattice::get_lattice_tree() const { return tree; }

Lattice_tree &Lattice::get_lattice_tree() { return tree; }

void Lattice::set_lattice_tree(Lattice_tree const &lattice_tree) {
  tree = lattice_tree;
}

void Lattice::set_variable(std::string const &name, double val) {
  tree.set_variable(name, val);
}

void Lattice::set_variable(std::string const &name, std::string const &val) {
  tree.set_variable(name, val);
}
